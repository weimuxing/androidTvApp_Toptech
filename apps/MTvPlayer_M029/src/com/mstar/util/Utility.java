...
public class Utility {
    ...
    public static boolean isSupportInputSourceLock() {
        ...
        return TvCommonManager.getInstance().isSupportModule(TvCommonManager.MODULE_INPUT_SOURCE_LOCK);
    }
    ...
    public static void doChannelSelected(Activity activity, int channelSelectNum, short serviceType) {
        TvChannelManager channelManager = TvChannelManager.getInstance();
        channelManager.selectProgram(channelSelectNum, serviceType);
        if (TvCommonManager.getInstance().getCurrentTvSystem() != TvCommonManager.TV_SYSTEM_ATSC) {
    ...
    public static void stopPvr(final Activity activity, boolean isForegroundRecording, boolean isPlaybacking) {
    ...
    //modify for select different channel program in channel list during recording,lxk 20150110
    public static void channelSelectedDialog(final Activity activity, final int channelSelectNum, final short serviceType) {
        final boolean isForegroundRecording = isForegroundRecording();
        final boolean isPlaybacking = TvPvrManager.getInstance().isPlaybacking();
        if ((true == isForegroundRecording) || (true == isPlaybacking)) {
            AlertDialog.Builder build = new AlertDialog.Builder(activity);
                    ...
                    new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            if(serviceType == TvChannelManager.SERVICE_TYPE_ATV){
                                /* do channel change until PVRActivity is not active.lxk 20150113*/
                                //Log.d(TAG,"----PVRActivity is active:" + PVRActivity.isPVRActivityActive);
                                new Thread(new Runnable() {
                                    @Override
                                    public void run() {
                                        try {
                                            ...
                                            Log.d(TAG,"--2--PVRActivity is active:" + PVRActivity.isPVRActivityActive);
                                            doChannelSelected(activity, channelSelectNum, serviceType);
                                        } catch (InterruptedException e) {
                                            e.printStackTrace();
                                        }
                                    }
