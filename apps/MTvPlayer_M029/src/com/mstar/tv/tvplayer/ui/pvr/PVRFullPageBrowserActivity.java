...
public class PVRFullPageBrowserActivity extends MstarBaseActivity {
    ...
    private void settingPasswardDialog() {
        mPasswordLock = new PasswordCheckDialog(this, R.layout.password_check_dialog_4_digits) {
            ...
            public void onPassWordCorrect() {
                Toast.makeText(mActivity, mActivity.getResources().getString(R.string.str_check_password_pass) , Toast.LENGTH_SHORT).show();
            ...
            public void onBackPressed() {
                Toast.makeText(mActivity, mActivity.getResources().getString(R.string.str_check_password_invalid) , Toast.LENGTH_SHORT).show();
                dismiss();
            }
        };
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mActivity = this;
        //add by wxy
        SystemProperties.set("persist.sys.pvrlist","1");
        //add end
    ...
    protected void onPause() {
        ...
        /*
         * When press home key will
         * 1. BrowserActivity onPause
         * 2. RootActivity onStop (change input source)
         * 3. BrowserActivity onStop
         * SetWindow before change input source,
         * prevent dtv resource be killed after change input source
         */
        stopPlaybacking();
        scaleToFullScreen();
        super.onPause();
    ...
    protected void onDestroy() {
        ...
        /* check input source, prevent goto PVRActivity input source not DTV */
        ...
        if (true == mPvrManager.isRecording()) {
		 ...
		 //add by wxy
			 SystemProperties.set("persist.sys.pvrlist","0");
		//add end
    }
        ...
        if (true == isPVRAvailable()) {
            switch (keyCode) {
                ...
                case KeyEvent.KEYCODE_PROG_BLUE: {
                    ...
                    //add by wxy for skip LCN sort
                    if(mCurSortKey == PvrManager.PVR_FILE_INFO_SORT_LCN){
                    	mCurSortKey = PvrManager.PVR_FILE_INFO_SORT_CHANNEL;
                    }
                    //add end
    ...
    private void playRecordedFile(int pos) {
        ...
        int recordDate = getRecordStartTime(pos);
        Date standRecordDate = new Date(recordDate);
        GregorianCalendar greCalendar = new GregorianCalendar();
        greCalendar.setTime(standRecordDate);
        java.text.SimpleDateFormat strFormat = new java.text.SimpleDateFormat("yyyy-MM-dd");
        String strRecordDate = strFormat.format(greCalendar.getTime());
        mTextLcnDataTime.setText(strRecordDate);
        Log.d(TAG, "file record start time = " + strRecordDate);
    ...
    private int getRecordStartTime(int index) {
        PvrFileInfo fileInfo = null;
        fileInfo = mPvrManager.getPvrFileInfo(index, mPvrManager.getMetadataSortKey());
        Log.d(TAG, "fileInfo.recordStartTime = " + fileInfo.recordStartTime);
        return fileInfo.recordStartTime;
    }
                ...
                while (!isFinishing()) {
                    mHandler.post(new Runnable() {
                        ...
                        public void run() {
                            ...
                            mPVRPlaybackProgress.setTextProgress(getTimeString(currentTime), currentTime);
                        }
                    });
                    Thread.sleep(1000);
                }
    ...
    //add by wxy
    private boolean showDeleteDialog(Context context)
    {
