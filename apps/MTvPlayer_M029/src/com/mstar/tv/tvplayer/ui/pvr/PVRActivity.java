...
public class PVRActivity extends MstarBaseActivity {
    ...
    public static boolean isPVRSelectingDev = false;
    private BroadcastReceiver mReceiver = null; //jerry 20160524 add
    private boolean isPvrPowerOff = false;
	private boolean isCreatedByEpg = false;
    private Intent intentChannelControl = null; //lcp 20160707 add
	...
	private BroadcastReceiver mtmpReceiver;
    ...
    private void findView() {
        ...
        pause.setOnFocusChangeListener(new OnFocusChangeListener(){
			...
			public void onFocusChange(View v, boolean hasFocus) {
				...
				if(hasFocus){ 
					if (mPvrManager.getPvrPlaybackSpeed() == mPvrManager.PVR_PLAYBACK_SPEED_STEP_IN)
					{
                   		pause.setImageResource(R.drawable.player_stepin_focus);		
					}
					else
					{
						pause.setImageResource(R.drawable.player_pause_focus);
					}
    ...
    private boolean isSupportedFileSystem(boolean isToastError) {
        if (false == recordDiskLable.regionMatches(6, FAT, 0, 3)) {
    ...
    private void settingUSB() {
        usbSelecter = new USBDiskSelecter(activity) {
            ...
            public void onSelectCancel() {
                Log.d(TAG, "onSelectCancel! Failed in starting record");
                isPVRSelectingDev = false;
                finish();
            }
        };
        usbSelecter.setUSBListener(new usbListener() {
            ...
            public void onUSBEject(String diskPath) {
                ...
                Toast.makeText(activity, activity.getResources().getString(R.string.str_disk_display), Toast.LENGTH_SHORT).show();
                finish();
            }
        });
    }
    ...
    private void settingPasswardDialog() {
        mPasswordLock = new PasswordCheckDialog(this, R.layout.password_check_dialog_4_digits) {
            ...
            public void onPassWordCorrect() {
                Toast.makeText(activity, activity.getResources().getString(R.string.str_check_password_pass) , Toast.LENGTH_SHORT).show();
            ...
            public void onBackPressed() {
                Toast.makeText(activity, activity.getResources().getString(R.string.str_check_password_invalid) , Toast.LENGTH_SHORT).show();
    ...
    private void toastErrorCode(int errorCode) {
        String[] dispInfo = getResources().getStringArray(
                R.array.str_arr_pvr_error_code);
        try {
    ...
    private boolean playfile(String pvrFileName) {
        ...
        int resumePlayTime = mPvrManager.getPvrFileResumePointInSec(pvrFileName, AVAILABLE_PERSON_INDEX);
        ...
        Log.d(TAG, "==========>>> playbackStatus = " + playbackStatus + "; resumePlayTime =" + resumePlayTime);
    ...
    protected void onCreate(Bundle savedInstanceState) {
		...
		SystemProperties.set("persist.sys.power2off","true");//add by hz2060804
        ...
        createAnimation();
        //jerry 20160108 add start
		mReceiver = new BroadcastReceiver() {
			@Override
			public void onReceive(Context context, Intent intent) {
				if (intent.getAction().equals("com.toptech.disable.recording")) {
					showPowerOffDialog();
				}
		...
		filter1.addAction("com.toptech.disable.recording");
		registerReceiver(mReceiver, filter1);
		//jerry 20160108 add end
		
		mtmpReceiver = new BroadcastReceiver() {
			@Override
			public void onReceive(Context context, Intent intent) {
				if (intent.getAction().equals("com.mstar.tv.tvplayer.pvr.disable_timeshift.start_record")) {
					if (curPvrMode == PVR_MODE.E_PVR_MODE_TIME_SHIFT) {
                        onKeyStopTimeShift();

                        if (TvCommonManager.getInstance().getCurrentInputSource().ordinal() != TvCommonManager.INPUT_SOURCE_DTV) {
                        ...
                        PVRActivity.this.startActivity(targetIntent);
                        Log.i(TAG,
		...
		filter.addAction("com.mstar.tv.tvplayer.pvr.disable_timeshift.start_record");
		registerReceiver(mtmpReceiver, filter);
		...
		isCreatedByEpg = (getIntent().getExtras().getInt(Constant.PVR_CREATE_FROM,0) == Constant.PVR_CREATE_FROM_EPG);
        Log.d(TAG, "mCreateMode:" + Integer.toHexString(mCreateMode));
        Log.d(TAG, "BrowserCall:" + mBackToBrowser);
        Log.d(TAG, "isCreatedByEpg:" + isCreatedByEpg);
        //add by wxy for creat from PVRBrowser do not lock
        if(mBackToBrowser)
        {
        ...
        if (isAlwaysTimeShiftRecording) {
            ...
            //add by jerry 2016024
            if(!mBackToBrowser) {
            TvCommonManager.getInstance().setTvosCommonCommand(TvCommonManager.TVOS_COMMON_CMD_AGING_START);
            SystemProperties.set("mstar.sys.recording", "1");
            }
            return;
        }

        if ((Constant.PVR_PLAYBACK_START == mCreateMode) || (Constant.PVR_PLAYBACK_FROM_BROWSER == mCreateMode)) {
        ...
        //add by jerry 2016024
        if(!mBackToBrowser && (!isPVRSelectingDev)) {
        TvCommonManager.getInstance().setTvosCommonCommand(TvCommonManager.TVOS_COMMON_CMD_AGING_START);
        SystemProperties.set("mstar.sys.recording", "1");
        }
    }
    ...
    public void onResume() {
        ...
        if ((false == mPvrManager.isRecording())
             && (false == mPvrManager.isPlaybacking())
             && (false == isPVRSelectingDev)) {
            /* Prevent goto browser Page, after close record or playbacking by other apps */
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        /*
         * The behavior is same as Pure SN.
         * Mheg5 is supported if PVR is in recording mode and the menu
         * is in hidden mode.
         *
         */
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_9:{
            	...
            	try {
            		...
            		final boolean isForegroundRecording = Utility.isForegroundRecording();
                    final boolean isPlaybacking = TvPvrManager.getInstance().isPlaybacking();
                    
                    intentChannelControl = new Intent(this, ChannelControlActivity.class);
                        ...
                        build.setPositiveButton(R.string.str_stop_record_dialog_stop,
                                new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                    	Utility.stopPvr(activity, isForegroundRecording, isPlaybacking);
                                        activity.startActivity(intentChannelControl);
                                    }
                                });
            ...
            case KeyEvent.KEYCODE_TIMESHIFT:
        ...
        /*
         * The behavior is same as Pure SN.
         * TTX is supported if PVR is in recording mode and the menu
         * is in hidden mode.
         *
         */
        if ((isMenuHide) && (PVR_MODE.E_PVR_MODE_RECORD == curPvrMode)) {
    ...
    private void showPowerOffDialog() {
        ...
        builder.setTitle(R.string.str_pvr_recording_poweroff_title);
        if(mPvrManager.isTimeShiftRecording())
        {
        	builder.setMessage(R.string.str_pvr_timeshif_poweroff_message);
        	Log.d(TAG,"IS_TIME_SHIFT = TURE");
        }
        else
        {
        	builder.setMessage(R.string.str_pvr_recording_poweroff_message);
        	Log.d(TAG,"IS_TIME_SHIFT = FALSE");
        }
        builder.setPositiveButton(R.string.str_pvr_recording_poweroff_positive_text,  new DialogInterface.OnClickListener(){
            @Override
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
				//modify by wxy for standby record
				saveAndExit();
				SystemProperties.set("persist.sys.power2off","false");
                SystemProperties.set("mstar.sys.recording", "0");
                SendKey(KeyEvent.KEYCODE_POWER);
				isPvrPowerOff = true;
				//end
            }
        });
        builder.setNegativeButton(R.string.str_pvr_recording_poweroff_nagative_text,  new DialogInterface.OnClickListener(){
            @Override
            public void onClick(DialogInterface dialog, int which) {
                dialog.dismiss();
            }
    ...
    protected void onStop() {
        ...
        //jerry add 201600524 start
        //modify by wxy catch the Exception when the receiver is not register or the receiver died 
        try {
        	unregisterReceiver(mReceiver);
        	unregisterReceiver(mtmpReceiver);
		} catch (Exception e) {
			// TODO: handle exception
			Log.i("wxy","unregisterReceiver Exception :"+e.getMessage());
		}
        //jerry add 201600524 end
    }
    ...
    protected void onDestroy() {
        Log.e(TAG, "onDestroy");
        //add by jerry 2016024
        TvCommonManager.getInstance().setTvosCommonCommand(TvCommonManager.TVOS_COMMON_CMD_AGING_STOP);
        SystemProperties.set("mstar.sys.recording", "0");
        ...
        //add by wxy
        if(isCreatedByEpg){
        	Toast.makeText(activity, R.string.str_stop_epg_record_toast, Toast.LENGTH_LONG).show();
        }
        //add end
        ...
        if (true == mBackToBrowser) {
            /* Prevent press home key, still go back to Browser (prevent no DTV resource)*/
    ...
    public void setBarStatusOfStartRecord() {
        ...
        if(mPvrManager.isTimeShiftRecording())
        {
        	mTextPlaybackStatusText.setText(R.string.str_pvr_is_TimeShift);
        	mImagePlaybackStatusImage.setImageResource(R.drawable.idle_img_press_ststus_timeshift);
        }
        else
        {
    ...
    private String getUsbLabelByPath(String diskPath) {
        ...
        for (int i = 0; i < volumes.length; ++i) {
            ...
            else
            {
            	...
            	volumeLabel = getString(R.string.str_removable_disk);
            }
    ...
    private void checkToStandbySystem() {
        ...
        if (true == isBootedByRecord) {
            ...
            if ((false == mPowerManager.isScreenOn())||(isPvrPowerOff)) {
				//add by wxy for standby record
    ...
    private void cancelValidEpgTimerEvent() {
        if (null != mTvTimerManager) {
            ...
            mTvTimerManager.cancelEpgTimerEvent((int) ((curTime.toMillis(true) / 1000) + 10 + 3), false);
        }
        ...
        if (mPvrManager.isTimeShiftRecording()) {
            if (mPvrManager.getPvrPlaybackSpeed() == TvPvrManager.PVR_PLAYBACK_SPEED_INVALID) {
                ...
                if (TvPvrManager.PVR_STATUS_SUCCESS != errorCode) {
                    ...
                    Log.e(TAG, "=========>>>> start(Always)TimeShiftPlayback ErrorCode: " + errorCode);
                    return;
                }
            } else {
            ...
            /*
             * Don't unmunt audio.
             * MW_DTV_AVMonitor::DoVideoSync will unmute video and audio after signal stable
             * mantis :0793483
             */
            unFreezeImage(false);
            mPauseTimeInSec = 0;
    ...
    public void onKeyStop() {
        ...
        if (PVR_MODE.E_PVR_MODE_PLAYBACK == curPvrMode) {
            ...
            dialog.setTitle(R.string.str_root_alert_dialog_title)
                    .setMessage(R.string.str_TimeShift_exit_confirm)
                    .setPositiveButton(R.string.str_root_alert_dialog_confirm,
                            new DialogInterface.OnClickListener() {
                                ...
                                public void onClick(DialogInterface dialog, int arg1) {
                                    dialog.dismiss();
                                    Log.d(TAG, "Click stop timeshift");
                                    stopPlaybacking();
                                    mPvrManager.stopTimeShiftRecord();
...
//            finish();
    ...
    public void onKeyPause() {
        ...
        if (curPvrMode == PVR_MODE.E_PVR_MODE_NONE)
            return;
        if ((curPvrMode == PVR_MODE.E_PVR_MODE_TIME_SHIFT) || (curPvrMode == PVR_MODE.E_PVR_MODE_ALWAYS_TIME_SHIFT)) {
    ...
    private void updateSpeedIcon(int speed) {
        ...
        switch (speed) {
            ...
            case TvPvrManager.PVR_PLAYBACK_SPEED_STEP_IN:
                playSpeed.setVisibility(View.GONE);
                playSpeed.setText("");
                mImagePlaybackStatusImage.setImageResource(R.drawable.player_pause);
                break;
    ...
    private void createIconAnimation() {
        ...
        ObjectAnimator IconfadeOutAlphaAnim = ObjectAnimator.ofFloat(mRelativeRecordingView.findViewById(R.id.pvrrecordimage),
                "alpha", 1f, 0f);
        ...
        IconfadeOutAlphaAnim.addListener(new AnimatorListener() {
            private int count = 0;
            private TextView text = (TextView) mRelativeRecordingView.findViewById(R.id.pvrrecordtext);
            private String strName = getResources().getString(mPvrManager.isTimeShiftRecording()?R.string.str_pvr_is_TimeShift:R.string.str_pvr_is_recording);
    ...
    private void createAnimation() {
        ...
        ObjectAnimator fadeOutAlphaAnim = ObjectAnimator.ofFloat(mRelativeRootView, "alpha", 1f, 0f);
        fadeOutAlphaAnim.setInterpolator(new DecelerateInterpolator());
        fadeOutAlphaAnim.setDuration(300);
        ObjectAnimator moveUpAnim = ObjectAnimator.ofFloat(mRelativeRootView, "translationY", height, 0);
        moveUpAnim.setInterpolator(new DecelerateInterpolator());
        moveUpAnim.setDuration(300);
        ObjectAnimator moveDownAnim = ObjectAnimator.ofFloat(mRelativeRootView, "translationY", 0, height);
    ...
    /*
     * update mCurrentTime or mTotalTime before using.
     * TODO: move mCurrentTime/mTotalTime to input arguments
     */
    private void updateProgress() {
        final boolean isPlaybacking = mPvrManager.isPlaybacking();
        final int currentTime = isPlaybacking ? mCurrentTime : 0;
        final int total = mTotalTime;
        Log.d(TAG, "==========>>> current: " + mCurrentTime+ "/  total: " +mTotalTime);
        handler.post(new Runnable() {
            @Override
            public void run() {
    ...
    private class TvPlayerEventListener implements OnTvPlayerEventListener {
        ...
        public boolean onPvrNotifyRecordTime(int what, int arg1) {
            /* When play not recording file don't update total time base on record time */
        ...
        public boolean onPvrNotifyPlaybackStop(int what) {
            ...
            activity.runOnUiThread( new Runnable() {
                public void run() {
                    /*
                     * PVR SPEC: Timeshift playback file end,
                     * jump to TIMESHIFT_PLAYBACK_DELAY_SECS secs before end of file and back to normal play.
                     * exp: FF/Jump to End, total time 100 secs, jump to 95 secs
                     * exp: FF/Jump to End, total time 3 secs, jump to 0 secs (if total time less then TIMESHIFT_PLAYBACK_DELAY_SECS)
                     */
                    final boolean isTimeShiftRecording = mPvrManager.isTimeShiftRecording();
                    if (true == isTimeShiftRecording) {
                        final String curPlaybackingFileName = mPvrManager.getCurPlaybackingFileName();
                        final int total = mPvrManager.getRecordedFileDurationTime(curPlaybackingFileName);
                        final int jumpToTimeInSeconds = ((total - TIMESHIFT_PLAYBACK_DELAY_SECS) > 0) ? (total - TIMESHIFT_PLAYBACK_DELAY_SECS) : 0;
                        playSpeed.setVisibility(View.GONE);
                        playSpeed.setText("");
                        mPvrManager.setPvrPlaybackSpeed(TvPvrManager.PVR_PLAYBACK_SPEED_1X);
    ...
    public void onKeyStopTimeShift() {
        Log.d(TAG, "onKeyStop curPvrMode:" + curPvrMode);
