...
public class ChannelListActivity extends MstarBaseActivity implements
    ...
    private TextView ch_list_exit=null;
    private TextView ch_list_backimage=null;
    ...
    public class ChannelListAdapter extends BaseAdapter {
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            ImageView favImage = (ImageView) convertView.findViewById(R.id.program_favorite_fav_img);
            ImageView lockImage = (ImageView) convertView.findViewById(R.id.program_lock_img);
            ImageView sslimage = (ImageView) convertView.findViewById(R.id.program_favorite_ssl_img);
           // if (mTvSystem == TvCommonManager.TV_SYSTEM_ATSC)   //del for all condition
            {
                ...
                switch(serviceType) {
                    ...
                    case TvChannelManager.SERVICE_TYPE_DATA:
                    	resId = R.drawable.list_menu_img_data_foucus;
                    	break;
            ...
            if (mData.get(position).isFavImg()) {
            	favImage.setVisibility(View.VISIBLE);
            } else {
            	favImage.setVisibility(View.GONE);
             
            }
            if (mData.get(position).isLockImg()&&TvParentalControlManager.getInstance().isSystemLock()) {
            ...
            if (pgi != null) {
                if (Constant.SHOW_FAVORITE_LIST == mProgListType) {
                    ...
                    if ((pgi.isDelete == true) || (pgi.isVisible == false) || pgi.favorite == 0) {
                        ...
                        pfo.setFavImg(pgi.favorite != 0);
                        pfo.setLockImg(pgi.isLock);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        ch_list_exit = (TextView) findViewById(R.id.ch_list_exit);
        ch_list_backimage = (TextView) findViewById(R.id.ch_list_backimage);
        if (getApp().isCusOnida()) {
        	ch_list_exit.setVisibility(View.GONE);
        	ch_list_backimage.setVisibility(View.VISIBLE);
		}
        ...
        mProgListView.setOnItemClickListener(new OnItemClickListener() {
			@Override
			public void onItemClick(AdapterView<?> parent, View view,
					int position, long id) {
				// TODO Auto-generated method stub
				final ProgramInfo progInfo = mProgInfoList.get(mAdapter.getListData().get(position).getProgInfoListIdx());
                Log.d(TAG, "number" + progInfo.number+"  position="+position);
                ...
                mAdapter.setFocusItem(position);
        ...
        mProgListView.setOnKeyListener(new OnKeyListener() {
            ...
            public boolean onKey(View view, int keyCode, KeyEvent keyEvent) {
                ...
                if (keyCode == KeyEvent.KEYCODE_ENTER
					...
					mAdapter.setFocusItem(selItemIndex);
                    mAdapter.notifyDataSetChanged();
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (keyCode == KeyEvent.KEYCODE_PROG_YELLOW) {
            if ((true == mIsSupportSearchAndSort) && (mState == EnumChannelListState.E_NORMAL_CHANNEL_LIST_MENU)) {
                mSearchView.setQuery("", false);
    ...
    private int getfocusIndex() {
        ...
        if (Constant.SHOW_PROGRAM_LIST == mProgListType
            || Constant.SHOW_FAVORITE_LIST == mProgListType) {
		...
		mAdapter.setFocusItem(focusIndex);
		mAdapter.notifyDataSetChanged();

        Log.v(TAG, "focusIndex = " + focusIndex);
        return focusIndex<0?0:focusIndex;
    }
