...
public class SignalInfoActivity extends MstarBaseActivity {
    ...
    private final static int SIGNAL_CLOSE_OSD = 0x02;
    ...
    private Handler mMessageHandler = new Handler() {
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
				...
				case SIGNAL_CLOSE_OSD:
					Log.d("wym", "close signal info\n");
					if (TvCommonManager.getInstance().getOsdDuration() != TvCommonManager.DURATION_INDEX_ALWAYS)
					finish();
					break;
                default:
                    break;
    ...
    protected void onResume() {
        ...
        mMessageHandler.sendEmptyMessageDelayed(SIGNAL_CLOSE_OSD, mTvCommonManager.getOsdTimeoutInSecond()*1000);
    }

    @Override
    protected void onPause() {
        mMessageHandler.removeMessages(DTV_REFRESH_UI);
        mMessageHandler.removeMessages(SIGNAL_CLOSE_OSD);
        super.onPause();
    }
    ...
    private void setChannelAndFrequencyInfo() {
        ...
        if (true == Utility.isATSC()) {
			...
			if(((double)rfInfo.frequency / 1000)>(rfInfo.frequency / 1000))
				str = "" + ((double)rfInfo.frequency / 1000);
			else
				str = "" + (rfInfo.frequency / 1000);
			int mFrequency;
			DvbMuxInfo dmi = mTvChannelManager.getCurrentMuxInfo();
			int mDvbsRouteIndex = mTvChannelManager
	                .getSpecificDtvRouteIndex(TvChannelManager.TV_ROUTE_DVBS);
			if (dmi != null) {
    ...
    private void setProgressBarValue(int qualityVal, int strengthVal) {
		...
		else if ((qualityVal > 0) && (0 == qualityVal/(PROGRESS_MAX + 1))) {
			qualityVal = 10;
		}
        if (strengthVal < PROGRESS_MIN) {
            strengthVal = PROGRESS_MIN;
        }
		else if ((strengthVal > 0) && (0 == strengthVal/(PROGRESS_MAX + 1))) {
			strengthVal = 10;
		}
