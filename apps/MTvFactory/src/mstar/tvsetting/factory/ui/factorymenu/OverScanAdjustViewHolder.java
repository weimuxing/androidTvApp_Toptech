...
public class OverScanAdjustViewHolder {
    ...
    public boolean onCreate() {
        ...
        if (factoryManager.getOverScanSourceType() != EnumInputSource.E_INPUT_SOURCE_VGA) {
            setSourceText(factoryManager.getOverScanSourceType().ordinal());
            sourceindex = validSources.indexOf(factoryManager.getOverScanSourceType());
        } else {
            setSourceText(validSources.get(sourceindex).ordinal());
    ...
    private void setSourceText(int inputSource) {
        String dispStr = sourcearray[inputSource];
        if ((TvCommonManager.INPUT_SOURCE_HDMI <= inputSource)
            && (TvCommonManager.INPUT_SOURCE_HDMI4 >= inputSource)) {
            dispStr = Tools.getHdmiDispStr(inputSource);
        }
        text_factory_overscan_source_val.setText(dispStr);
    }
}
