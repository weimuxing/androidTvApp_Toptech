...
public class FactorySoundViewHolder {
    ...
    protected LinearLayout  linearlayout_factory_non_linear_sound;

	protected LinearLayout  linearlayout_factory_peq;
	
    protected LinearLayout  linearlayout_factory_prescale;
    protected TextView textvalue_factory_prescale;
    ...
    void findView() {
        linearlayout_factory_non_linear_sound = (LinearLayout) SoundActivity
                .findViewById(R.id.linearlayout_factory_non_linear_sound);

		linearlayout_factory_peq = (LinearLayout) SoundActivity
                .findViewById(R.id.linearlayout_factory_peq);
		
        linearlayout_factory_prescale = (LinearLayout) SoundActivity
                .findViewById(R.id.linearlayout_factory_prescale);
        textvalue_factory_prescale = (TextView) SoundActivity
                .findViewById(R.id.textvalue_factory_prescale);

        linearlayout_factory_lineout = (LinearLayout) SoundActivity
                .findViewById(R.id.linearlayout_factory_lineout);
        textvalue_factory_lineout = (TextView) SoundActivity
                .findViewById(R.id.textvalue_factory_lineout);

        linearlayout_factory_avc_enable = (LinearLayout) SoundActivity
                .findViewById(R.id.linearlayout_factory_avc_enable);
        textvalue_factory_avc_enable = (TextView) SoundActivity
                .findViewById(R.id.textvalue_factory_avc_enable);

        linearlayout_factory_avc_threthold = (LinearLayout) SoundActivity
                .findViewById(R.id.linearlayout_factory_avc_threthold);
        textvalue_factory_avc_threthold = (TextView) SoundActivity
                .findViewById(R.id.textvalue_factory_avc_threthold);

        linearlayout_factory_drc_enable = (LinearLayout) SoundActivity
                .findViewById(R.id.linearlayout_factory_drc_enable);
        textvalue_factory_drc_enable = (TextView) SoundActivity
                .findViewById(R.id.textvalue_factory_drc_enable);

        linearlayout_factory_drc_threthold = (LinearLayout) SoundActivity
                .findViewById(R.id.linearlayout_factory_drc_threthold);
        textvalue_factory_drc_threthold = (TextView) SoundActivity
                .findViewById(R.id.textvalue_factory_drc_threthold);
        ...
        textvalue_factory_audio_delay = (TextView) SoundActivity
                .findViewById(R.id.textvalue_factory_audio_delay);

        linearlayout_factory_audio_nr = (LinearLayout) SoundActivity
                .findViewById(R.id.linearlayout_factory_audio_nr);
        textvalue_factory_audio_nr = (TextView) SoundActivity
                .findViewById(R.id.textvalue_factory_audio_nr);
		...
		textvalue_factory_speaker = (TextView) SoundActivity
                .findViewById(R.id.textvalue_factory_speaker);
	...
	private void initValues()
	{
        ...
        {
			//speaker_prescale = am.getBasicSoundEffect(EnumSoundGetParameterType.E_PRESCALE);
			//avc_enable = am.getBasicSoundEffect(EnumSoundGetParameterType.E_AVC_ONOFF)?true:false;
			//avc_threthold = am.getBasicSoundEffect(EnumSoundGetParameterType.E_AVC_THRESHOLD);
			//drc_enable = am.getBasicSoundEffect(EnumSoundGetParameterType.E_PRESCALE)?true:false;
			//drc_threthold = am.getBasicSoundEffect(EnumSoundGetParameterType.E_DRC_THRESHOLD);
			//audio_delay = am.getBasicSoundEffect(EnumSoundGetParameterType.E_PRESCALE);
			//audio_nr = am.getBasicSoundEffect(EnumSoundGetParameterType.E_NR_THRESHOLD);
        }
	}
	
	private void setValues()
	{
		textvalue_factory_prescale.setText("0x"+Integer.toHexString(speaker_prescale));
    ...
    private void registerListeners() {
        this.linearlayout_factory_non_linear_sound.setOnClickListener(listener);
        this.linearlayout_factory_peq.setOnClickListener(listener);
    }
    ...
    private OnClickListener listener = new OnClickListener() {
        ...
        public void onClick(View view)
        {
			switch (view.getId())
			{
				case R.id.linearlayout_factory_non_linear_sound:
					Log.d(TAG, "linearlayout_factory_non_linear_sound clicked.");
					SoundActivity.nonlineartype = 0;// set to volume
					SoundActivity.gotoPage(/*NON_LINER_PAGE*/2);
					break;
				case R.id.linearlayout_factory_peq:
					Log.d(TAG, "linearlayout_factory_peq clicked.");
					SoundActivity.gotoPage(/*PEQ_PAGE*/5);
					break;
				default:
					Log.d(TAG, "view.getId() is " + view.getId());
					break;
			}
        }
    };
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        switch (keyCode) {
            ...
            case KeyEvent.KEYCODE_DPAD_LEFT:
            	{
					switch (currentid)
					{
						case R.id.linearlayout_factory_prescale:
						{
							if (speaker_prescale > 0) speaker_prescale -= 1;
							else speaker_prescale = 0xFF;
							// set then readback...
							ret_values = TvCommonManager.getInstance().setTvosCommonCommand(CMD_SET_PRESCALE+"#"+speaker_prescale);
							speaker_prescale = ret_values[0];
						}
							break;
						case R.id.linearlayout_factory_avc_enable:
						{
							...
							// set then readback...
							ret_values = TvCommonManager.getInstance().setTvosCommonCommand(CMD_SET_AVC_ENABLE+"#"+bti(avc_enable));
							avc_enable = (ret_values[0]>0)?true:false;
						}
							break;
						case R.id.linearlayout_factory_avc_threthold:
						{
							if (avc_threthold > 0) avc_threthold -= 1;
							else avc_threthold = 0xFF;
						...
						case R.id.linearlayout_factory_drc_enable:
						{
							...
							// set then readback...
							ret_values = TvCommonManager.getInstance().setTvosCommonCommand(CMD_SET_DRC_ENABLE+"#"+bti(drc_enable));
							drc_enable = (ret_values[0]>0)?true:false;
						}
							break;
						case R.id.linearlayout_factory_drc_threthold:
						{
							if (drc_threthold > 0) drc_threthold -= 1;
							else drc_threthold = 0xFF;
							// set then readback...
						...
						case R.id.linearlayout_factory_audio_delay:
						{
							...
							else audio_delay = 0xFF;
							// set then readback...
							ret_values = TvCommonManager.getInstance().setTvosCommonCommand(CMD_SET_AUDIO_DELAY+"#"+audio_delay);
							audio_delay = ret_values[0];
						}
							break;
						case R.id.linearlayout_factory_audio_spdif_delay:
						{
							if (audio_spdif_delay > 0) audio_spdif_delay -= 1;
							else audio_spdif_delay = 0xFF;
							// set then readback...
							ret_values = TvCommonManager.getInstance().setTvosCommonCommand(CMD_SET_AUDIO_SPDIF_DELAY+"#"+audio_spdif_delay);
							audio_spdif_delay = ret_values[0];
						}
							break;
						case R.id.linearlayout_factory_audio_nr:
						{
							if (audio_nr > 0) audio_nr -= 1;
							else audio_nr = 0xFF;
							// set then readback...
							...
							break;
						case R.id.linearlayout_factory_sound_effect:
						{// in fact it is hpf.........
						...
						case R.id.linearlayout_factory_sound_toneffect:
						{
							...
							// set then readback...
							ret_values = TvCommonManager.getInstance().setTvosCommonCommand("SetAudioToneEffect#"+bti(sound_tone_effect));
            ...
            case KeyEvent.KEYCODE_DPAD_RIGHT:
            	{
					switch (currentid)
					{
						case R.id.linearlayout_factory_prescale:
						{
							if (speaker_prescale < 0xFF) speaker_prescale += 1;
							else speaker_prescale = 0;
							// set then readback...
						...
						case R.id.linearlayout_factory_avc_threthold:
						{
							if (avc_threthold < 0xFF) avc_threthold += 1;
							else avc_threthold = 0;
						...
						case R.id.linearlayout_factory_drc_threthold:
						{
							if (drc_threthold < 0xFF) drc_threthold += 1;
							else drc_threthold = 0;
							// set then readback...
						...
						case R.id.linearlayout_factory_audio_delay:
						{
							if (audio_delay < 0xFF) audio_delay += 1;
							else audio_delay = 0;
							// set then readback...
						...
						case R.id.linearlayout_factory_audio_spdif_delay:
						{
							if (audio_spdif_delay < 0xFF) audio_spdif_delay += 1;
							else audio_spdif_delay = 0;
							// set then readback...
							ret_values = TvCommonManager.getInstance().setTvosCommonCommand("SetAudioSpdifDelay#"+audio_spdif_delay);
							audio_spdif_delay = ret_values[0];
						}
							break;
						case R.id.linearlayout_factory_audio_nr:
						{
							if (audio_nr < 0xFF) audio_nr += 1;
							else audio_nr = 0;
							// set then readback...
						...
						{
							int tmpval = (lineout_prescale&0x7FFF)>>5;
							if (tmpval < (0x7f<<3 | 0x07)) tmpval += 1;
							else tmpval = 0;
							lineout_prescale = tmpval<<5;
							// set then readback...
							ret_values = TvCommonManager.getInstance().setTvosCommonCommand("SetAudioLineout#"+lineout_prescale);
            ...
            case KeyEvent.KEYCODE_ENTER:
            	...
            	Log.d(TAG, "peq = "+R.id.linearlayout_factory_peq);
				if (currentid == R.id.linearlayout_factory_peq)
				{
					Log.d(TAG, "linearlayout_factory_peq.");
					SoundActivity.gotoPage(/*PEQ_PAGE*/5);
				}
            	break;
            default:
