...
public class FactoryMenuActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //end
        scrollx = 0;
        menulayout = (LinearLayout) findViewById(R.id.mstarmenu_layout);
    }
    ...
    protected void onResume() {
        ...
        Log.d("zhongbin","show infoccc");
      //zb20151231 add
  		if(intent != null)
  		{
  			Log.d("zhongbin","show infoxxx");
  			String hotkey_version=intent.getStringExtra("hotkey_version");
  			if(hotkey_version != null && hotkey_version.equals("1"))
  			{
  				Log.d("zhongbin","show info");
  				  mstarCurrentPage = Mstar_INFO_PAGE;
                  factoryViewFlipper.setDisplayedChild(Mstar_INFO_PAGE);
                  infoHolder = new InfoViewHolder(FactoryMenuActivity.this,factoryDesk);
                  infoHolder.findView();
                  infoHolder.onCreate();
  			}
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if (keyCode == KeyEvent.KEYCODE_PROG_RED) {
        	if (factoryViewFlipper.getVisibility() == View.VISIBLE)
			factoryViewFlipper.setVisibility(View.INVISIBLE);
			else
			factoryViewFlipper.setVisibility(View.VISIBLE);
			return true;
        } else if (factoryViewFlipper.getVisibility() != View.VISIBLE) {
			return true;
        }

        if (keyCode == KeyEvent.KEYCODE_PROG_GREEN) {
			//50dp 100dp
			int totaloffset = menulayout.getMeasuredWidth() - factoryViewFlipper.getMeasuredWidth();
			if (scrollx == 0) scrollx = totaloffset/2;//-Math.round(50*density);
			else if (scrollx == totaloffset/2) scrollx = totaloffset;//-Math.round(100*density);
			else scrollx = 0;
			menulayout.scrollTo(-scrollx, 0);
			return true;
        }
