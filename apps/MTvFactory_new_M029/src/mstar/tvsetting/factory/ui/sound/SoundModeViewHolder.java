...
public class SoundModeViewHolder {
	...
	public boolean onCreate() {
		...
		if(soundModeActivity.isM029()){
			soundmodetypearray = soundModeActivity.getResources().getStringArray(R.array.str_arr_sound_soundmode_m029_vals);
		} else {
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		case KeyEvent.KEYCODE_DPAD_RIGHT:
			...
			case R.id.linearlayout_sound_mode:
				if (type < (soundmodetypearray.length - 1))
					type++;
				else
					type = 0;
		...
		case KeyEvent.KEYCODE_DPAD_LEFT:
			...
			case R.id.linearlayout_sound_mode:
				if (type > 0)
					...
					type = soundmodetypearray.length - 1;
				tvAudioManager.setAudioSoundMode(type);
				freshDataToUIWhenSoundModeChange();
				return true;
			case R.id.linearlayout_sound_mode_bass:
	...
	private void freshDataToUIWhenSoundModeChange() {
		...
		if(type >= soundmodetypearray.length){
			type = 0;
			tvAudioManager.setAudioMode(type);
		}
