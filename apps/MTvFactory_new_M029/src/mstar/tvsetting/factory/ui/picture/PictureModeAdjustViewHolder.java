...
public class PictureModeAdjustViewHolder {
	...
	private int factoryPicModeIndex = 0;
	...
	private final int MENU_PIC_MODE_LIGHTNESS_INDEX = 0;
	private final int MENU_PIC_MODE_FLAT_INDEX = 1;
	private final int MENU_PIC_MODE_CINEMA_INDEX = 2;
	private final int MENU_PIC_MODE_NORMAL_INDEX = 3;
	private final int MENU_PIC_MODE_VIVID_INDEX = 7;
	private final int MENU_PIC_MODE_SPORTS_INDEX = 9;
	
	private final int FACTORY_PIC_MODE_LIGHTNESS_INDEX = 0;
	private final int FACTORY_PIC_MODE_FLAT_INDEX = 1;
	private final int FACTORY_PIC_MODE_CINEMA_INDEX = 2;
	private final int FACTORY_PIC_MODE_NORMAL_INDEX = 3;
	private final int FACTORY_PIC_MODE_VIVID_INDEX = 0;
	private final int FACTORY_PIC_MODE_SPORTS_INDEX = 4;
	...
	public PictureModeAdjustViewHolder(DesignMenuActivity activity, IFactoryDesk factoryManager) {
		...
		if(picModeActivity.isM029()){
			mPicModeStrArray = picModeActivity.getResources().getStringArray(R.array.str_arr_pic_mode_m029_vals);
		} else {
			mPicModeStrArray = picModeActivity.getResources().getStringArray(R.array.str_arr_pic_mode_vals);
		}
	}
	...
	public void findView() {
		...
		text_factory_picmode_hue_val = (TextView) picModeActivity.findViewById(R.id.textview_factory_picmode_hue_val);
		...
		progress_factory_picmode_hue = (ProgressBar) picModeActivity.findViewById(R.id.progressbar_facroty_picmode_hue);
	...
	public boolean onCreate() {
		...
		factoryPicModeIndex = getFactoryPicModeIndex(picmodeindex);
		if(factoryPicModeIndex >= mPicModeStrArray.length){
			factoryPicModeIndex = 0;
			picmodeindex = getMenuPicModeIndex(factoryPicModeIndex);
		...
		text_factory_picmode_picmode_val.setText(mPicModeStrArray[factoryPicModeIndex]);
	...
	private int getFactoryPicModeIndex(int menuPicModeIndex){
		int index = 0;
		switch (menuPicModeIndex) {
		case MENU_PIC_MODE_LIGHTNESS_INDEX:
			index = FACTORY_PIC_MODE_LIGHTNESS_INDEX;
			break;
		case MENU_PIC_MODE_FLAT_INDEX:
			index = FACTORY_PIC_MODE_FLAT_INDEX;
			break;
		case MENU_PIC_MODE_CINEMA_INDEX:
			index = FACTORY_PIC_MODE_CINEMA_INDEX;
			break;
		case MENU_PIC_MODE_NORMAL_INDEX:
			index = FACTORY_PIC_MODE_NORMAL_INDEX;
			break;
		case MENU_PIC_MODE_VIVID_INDEX:
			index = FACTORY_PIC_MODE_VIVID_INDEX;
			break;
		case MENU_PIC_MODE_SPORTS_INDEX:
			index = FACTORY_PIC_MODE_SPORTS_INDEX;
			break;
		}
		return index;
	}
	
	private int getMenuPicModeIndex(int factoryPicModeIndex){
		int index = 0;
		if(factoryPicModeIndex == 0){
			index = picModeActivity.isM029() ? MENU_PIC_MODE_VIVID_INDEX : MENU_PIC_MODE_LIGHTNESS_INDEX;
		}
		switch (factoryPicModeIndex) {
		case FACTORY_PIC_MODE_FLAT_INDEX:
			index = MENU_PIC_MODE_FLAT_INDEX;
			break;
		case FACTORY_PIC_MODE_CINEMA_INDEX:
			index = MENU_PIC_MODE_CINEMA_INDEX;
			break;
		case FACTORY_PIC_MODE_NORMAL_INDEX:
			index = MENU_PIC_MODE_NORMAL_INDEX;
			break;
		case FACTORY_PIC_MODE_SPORTS_INDEX:
			index = MENU_PIC_MODE_SPORTS_INDEX;
			break;
		}
		return index;
	}
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		...
		case KeyEvent.KEYCODE_DPAD_RIGHT:
			...
			case R.id.linearlayout_factory_picmode_picmode:
				...
				maxValue = mPicModeStrArray.length;
				if (factoryPicModeIndex < (maxValue - 1)) {
					/*if (currentInputSrc == mTvCommonManager.INPUT_SOURCE_VGA
						...
						if (index == EN_MS_PICTURE.PICTURE_GAME.ordinal()) {
							index = EN_MS_PICTURE.PICTURE_VIVID.ordinal();
						} else {
							index++;
						}
					}*/
			...
			case R.id.linearlayout_factory_picmode_picmode:
				...
				if (factoryPicModeIndex > 0) {
					/*if (currentInputSrc == mTvCommonManager.INPUT_SOURCE_VGA
						...
						if (index == EN_MS_PICTURE.PICTURE_VIVID.ordinal()) {
							index = EN_MS_PICTURE.PICTURE_GAME.ordinal();
						} else {
							index--;
						}
					}*/
					...
					factoryPicModeIndex = maxValue - 1;
				}
				text_factory_picmode_picmode_val.setText(mPicModeStrArray[factoryPicModeIndex]);
				picmodeindex = getMenuPicModeIndex(factoryPicModeIndex);
				mTvPictureManager.setPictureMode(picmodeindex);
