...
public class PqColorQmapAdapter extends BaseAdapter {
    private ArrayList<PqViewHolder> mData = null;

    Activity act = null;

    public PqColorQmapAdapter(Activity activity, boolean initNow) {
        act = activity;
        mData = new ArrayList<PqViewHolder>();
        mData.clear();
        if (initNow) {
            int ipNums = TvFactoryManager.getInstance().getQmapCustomerIpNum();
            for (int i = 0; i < ipNums; i++) {
                PqViewHolder pvh = new PqViewHolder(Constant.PQ_TABLE_TYPE_LITTLE, i);
                pvh.init();
                mData.add(pvh);
            }
        }
