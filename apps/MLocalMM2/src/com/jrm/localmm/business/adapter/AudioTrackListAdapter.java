...
import com.jrm.localmm.ui.video.AudioTrackManager;
import com.jrm.localmm.ui.video.PlaySettingAudioTrackDialog;
import com.jrm.localmm.ui.video.VideoPlayerActivity;
import com.jrm.localmm.util.Constants;

/**
 *
 * @author zhangpeng(zhangpeng@biaoqi.com.cn)
 *
 * @since 1.0
 *
 * @date 2012-2-20
...
public class AudioTrackListAdapter extends BaseAdapter {
    ...
    public AudioTrackListAdapter(Context context, int[] settingData,
            String[] optionData, PlaySettingAudioTrackDialog playSettingAudioTrackDialog) {
        try {
            ...
            if (Constants.bUseAndroidStandardMediaPlayerTrackAPI) {
                count = videoPlayActivity.getVideoPlayHolder().getPlayerView(videoPlayActivity.getVideoPlayHolder().
                        getViewId()).getMsAudioTrackCount();
            } else {
                count = AudioTrackManager.getInstance().getAudioTrackCount(videoPlayActivity.getVideoPlayHolder().getPlayerView().getMMediaPlayer());
            }
            this.mPlaySettingAudioTrackDialog = playSettingAudioTrackDialog;
        } catch (Exception e) {
        }
    }
    ...
    public View getView(int position, View convertView, ViewGroup arg2) {
        if (convertView == null) {
            ...
            if (mPlaySettingAudioTrackDialog != null) {
        ...
        if (position == 0) {
            if (count == 0) {
                holder.settingOptionTextView.setText(videoPlayActivity
                        .getResources().getString(R.string.no_video_track));
            }
        } /*else if (position == 1) {
            holder.settingOptionTextView.setText(String
                    .valueOf(count));
        } else if (position == 2) {
            if (count != 0){
                int iDuration = videoPlayActivity.getAudioTrackDuration();
                if (iDuration >= 1000) {
                    iDuration /= 1000;
                    String strFormatDuration = String.format("%02d:%02d:%02d",
                            iDuration / 3600, iDuration % 3600 / 60,
                            iDuration % 60);
                    holder.settingOptionTextView.setText(strFormatDuration);
                }
            }
        }*/
        return convertView;
    ...
    private void setClickListens(final int positon) {
        holder.rightImageView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                mPlaySettingAudioTrackDialog.handleImageClick(positon, false);
            }
        });
        holder.leftImageView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                mPlaySettingAudioTrackDialog.handleImageClick(positon, true);
            }
        });
        holder.settingOptionTextView.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                //mPlaySettingAudioTrackDialog.handleMidClick(positon);
            }
        });
