...
public class ReproduceRate {
    ...
    public void setReproduceRate() {
        this.scaleval = (_reduceRate.topRate << 24) + (_reduceRate.bottomRate << 16) + (_reduceRate.leftRate << 8) + (_reduceRate.rightRate);
        int hw[] = new int[2];
        getResolutionInfo(hw);
        pictureSkin.setSurfaceResolutionMode(hw[0], hw[1], hstart, _reduceRate.interleave, direction.ordinal(), this.scaleval);
    }
