...
package com.mstar.miscsetting.broadcast;
...
/**
 * HotKey radio receiver (registered way for the system level registered)
 * @author zhoujf
 */
public class HotKeyBroadCastReceiver extends BroadcastReceiver {

    private static final String TAG = "HotKeyBroadCastReceiver";
    @Override
    public void onReceive(Context context, Intent intent) {

        String action = intent.getAction();
        if (action.equals("enter.4k2k")) {
            Log.i(TAG, "--------------------------->enter.4k2k! " + MainMenuHolder.miscmode);
            switch (MainMenuHolder.miscmode) {
                case PIP_MODE:
                    swithVol2Main(context);
    ...
    private void swithVol2Main(Context context) {
        ...
        TvAudioManager.getInstance().setEarPhoneVolume(speakerVolume);
        if (PipService.volStatus != SWITCH_VOL.BT_HEADSET_MAIN) {
            ...
            Tools.toastShow(R.string.switch_main, context);
