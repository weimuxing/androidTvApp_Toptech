...
public class TvResolutionUtil {
    ...
    public final String ENABLE_4K2K_UI_PRIORITY = "mstar.resolution.4k2kEnableUI";
    ...
    private TvResolutionUtil() {
        ...
        for (int i = 0; i < timingList.length; i++) {
            resolutionArray[i] = timingList[i].hResolution + "x" + timingList[i].vResolution
                    + (timingList[i].progressiveMode ? "P" : "I") + "@" + timingList[i].frameRate
                    + "Hz";
        }
    }
    ...
    public void setCurrentResolution(int index, Context mContext) {
        Log.d(TAG, "setCurrentResolution: " + resolutionArray[index] + ", timingID = "
                + timingList[index].timingID);
        ...
        pictureSkin.Connect();
        pictureSkin.setSurfaceResolutionMode(timingList[index].hResolution,
                timingList[index].vResolution, getHStart(index),
    ...
    protected void show4K2KNotSupportDialog(Context mContext) {
        ...
        builder.setPositiveButton(R.string.str_ok_label, new OnClickListener() {
            @Override
