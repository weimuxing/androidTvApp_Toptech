...
public class KeyboardView extends View implements PointerTracker.DrawingProxy {
    ...
    private static int mLastKeyIndex = -1;
    private static String mLastPrintableKeyCode;
    private Rect rect;
    ...
    public void resetKeyFocus() {
        if (mKeyboard!=null && !(mKeyboard instanceof MoreKeysKeyboard)) {
            Key[] allkeys = mKeyboard.mKeys;
            if (mKeyboard.mId.isNumberKeyboard()) {
                for (int i=0; i<allkeys.length; i++) {
                    if (allkeys[i].mCode=='1') {
                        setLastKeyIndex(i);
                        return;
                    }
                }
                ...
                for (int i=0; i<allkeys.length; i++) {
                    if (mLastPrintableKeyCode!=null && mLastPrintableKeyCode.equalsIgnoreCase(Keyboard.printableCode(allkeys[i].mCode))) {
                        setLastKeyIndex(i);
                        Log.d(TAG, "resetKeyFocus to "+allkeys[i].mLabel);
                        return;
                    }
                }
                for (int i=0; i<allkeys.length; i++) {
                    if (allkeys[i].mCode==Keyboard.CODE_ACTION_ENTER || allkeys[i].mCode==Keyboard.CODE_ENTER) {
                        setLastKeyIndex(i); // reset key focus to enter key.
                        Log.d(TAG, "resetKeyFocus to enter key");
                        return;
                    }
                }
            }
    ...
    public void onDraw(final Canvas canvas) {
        ...
        if (!(mKeyboard instanceof MoreKeysKeyboard)) {
            Key focusedKey = mKeyboard.mKeys[mLastKeyIndex];
            Paint p = new Paint();
            p.setColor(Color.CYAN);
            p.setStyle(Paint.Style.STROKE);
            p.setStrokeWidth(3.75f);
            rect = new Rect(
                focusedKey.mX, focusedKey.mY + 2,
    ...
    public void setLastKeyIndex(int index) {
        mLastKeyIndex = index;
        mLastPrintableKeyCode = Keyboard.printableCode(mKeyboard.mKeys[mLastKeyIndex].mCode);
    }
    ...
    private void locatePreviewPlacerView() {
        ...
        viewOrigin[0] = (dm.widthPixels-width)/2;
        viewOrigin[1] = dm.heightPixels-height;
