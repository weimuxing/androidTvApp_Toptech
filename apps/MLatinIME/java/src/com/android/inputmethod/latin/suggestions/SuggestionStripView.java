...
public final class SuggestionStripView extends RelativeLayout implements OnClickListener,
    ...
    private static final class SuggestionStripViewParams {
        ...
        public void layout(SuggestedWords suggestedWords, ViewGroup stripView, ViewGroup placer,
            ...
            for (int index = 0; index < countInStrip; index++) {
                ...
                if (text == null) {
					...
					word.setText(String.valueOf(++wordIndex).concat(". ").concat(text.toString())); // TextView.setText() resets text scale x to 1.0.
				}
        ...
        private void layoutPunctuationSuggestions(SuggestedWords suggestedWords,
            ...
            for (int index = 0; index < countInStrip; index++) {
                ...
                if (text == null) {
					...
					word.setText(String.valueOf(++wordIndex).concat(". ").concat(text.toString()));
				}
        ...
        public void layoutAddToDictionaryHint(CharSequence word, ViewGroup stripView,
            ...
            if (text == null) {
				...
				wordView.setText(String.valueOf(1).concat(". ").concat(text.toString()));
			}
    ...
    public boolean selectSuggestions(int index) {
    	if (mWords == null || mWords.isEmpty()) {
    		return false;
    	}
    	if (mSuggestedWords == null || mSuggestedWords == SuggestedWords.EMPTY) {
    		return false;
    	}
		if (mWords.size() == 0 || mSuggestedWords.size() == 0) {
    		return false;
    	}
		
		int size = mSuggestedWords.size();
    	...
    	if (index < 0) {
    		...
    		position = mParams.getWordPosition(index, mSuggestedWords);
    	}
    	
        if (position >= 0 && position < size && position < mParams.mSuggestionsCountInStrip) {
            CharSequence text = mSuggestedWords.getWord(position);
            mListener.pickSuggestionManually(position, text);
        }
		return true;
    }
    
    public boolean addToDictionary(int index) {
    	if (mSuggestionsStrip == null || mParams.mWordToSaveView == null) {
    		return false;
    	}
    	int i = mSuggestionsStrip.indexOfChild(mParams.mWordToSaveView);
    	if (i == -1) {
    		return false;
    	}
