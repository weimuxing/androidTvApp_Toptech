...
public final class LatinIME extends InputMethodService implements KeyboardActionListener,
    ...
    private static final int INVALID_KEYUP_INTERVAL = 30;
    ...
    //fields added for remote control
    private int nKeyNums;
    private int nLastKeyIndex;
    private int nearestIndex;
    private Key lastKey;
    private Key nearestKey;
    private Key nkey;
    private KeyboardView nCurrentView;
    private Keyboard nCurrentKeyboard;
    private Key[] nKeys;

    private Key lastMoreKey;
    private boolean isOnKeyLongPress = false;
    private int keyCodeOnKeyDown = KeyEvent.KEYCODE_UNKNOWN;
    ...
    // set fields for remote control
    private void setFields() {
        nCurrentKeyboard = nCurrentView.getKeyboard();
        nKeys = nCurrentKeyboard.mKeys;
        nKeyNums = nKeys.length;
        nLastKeyIndex = nCurrentView.getLastKeyIndex();
        nearestIndex = nLastKeyIndex;
        if(nLastKeyIndex>nKeys.length-1)
            nLastKeyIndex = nKeys.length-1;
        lastKey = nKeys[nLastKeyIndex];
    }
    ...
    public boolean onKeyLongPress(int keyCode, KeyEvent event) {
        if (keyCode==KeyEvent.KEYCODE_ENTER || keyCode==KeyEvent.KEYCODE_DPAD_CENTER) {
            setFields();
            if (lastKey.mCode==Keyboard.CODE_SWITCH_ALPHA_SYMBOL || lastKey.mCode==Keyboard.CODE_SHIFT) {
                return true;
            }
            isOnKeyLongPress = true;
            final PointerTracker tracker = PointerTracker.getPointerTracker(0, (MainKeyboardView)nCurrentView);
            tracker.onDownEvent(lastKey.mX, lastKey.mY, SystemClock.uptimeMillis(), (MainKeyboardView)nCurrentView);
            MainKeyboardView mainKeyboardView = mKeyboardSwitcher.getMainKeyboardView();
            mainKeyboardView.openMoreKeysKeyboardIfRequired(lastKey);
            lastMoreKey = tracker.getKey(); // get the defaut moreKey
            return true;
        }
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        // do not handle onKeyUp event after long press
        nCurrentView = mKeyboardSwitcher.getMainKeyboardView();
        if ((nCurrentView != null)&&(nCurrentView.isShown())) {
            final PointerTracker tracker = PointerTracker.getPointerTracker(0, (MainKeyboardView)nCurrentView);
            if (isOnKeyLongPress) {
                if (((MainKeyboardView)nCurrentView).hasMoreKeysPanel()) {
                    Keyboard moreKeysKeyboard = tracker.getKeyboard();
                    if (moreKeysKeyboard.mKeys.length==1) { // only one moreKey, e.g: t/5, y/6
                        tracker.onUpEvent(lastKey.mX, lastKey.mY, SystemClock.uptimeMillis());
                    }
                    // do nothing, if there exist more than one moreKey, so that the MoreKeysKeyboard will keep on screen.
                }
                else { // no moreKeys, e.g: j, k, ...
                    tracker.onUpEvent(lastKey.mX, lastKey.mY, SystemClock.uptimeMillis());
                }
                isOnKeyLongPress = false;
                return true;
            } else if (!isOnKeyLongPress && ((MainKeyboardView)nCurrentView).hasMoreKeysPanel()) { // the MoreKeysKeyboard is showing.
                Keyboard moreKeysKeyboard = tracker.getKeyboard();
                nKeys = tracker.getKeyboard().mKeys;
                nKeyNums = nKeys.length;

                switch(keyCode) {
                case KeyEvent.KEYCODE_ENTER:
                case KeyEvent.KEYCODE_DPAD_CENTER:
                    tracker.onUpEvent(lastMoreKey.mX, lastMoreKey.mY, SystemClock.uptimeMillis());
                    break;
                }
            }
            else {
                nCurrentKeyboard = nCurrentView.getKeyboard();
                //nCurrentKeyboard.mOccupiedHeight = 420;
                nKeys = nCurrentKeyboard.mKeys;
                nKeyNums = nKeys.length;
                nLastKeyIndex = nCurrentView.getLastKeyIndex();
                nearestIndex = nLastKeyIndex;
                ...
                case KeyEvent.KEYCODE_DPAD_CENTER:
                    ...
                    setFields();
                    int curKeycode = lastKey.mCode;
                    if (curKeycode!=keyCodeOnKeyDown) {// to cancel first keyUp event when keyboard popup
                        return true;
                    }
                    keyCodeOnKeyDown = KeyEvent.KEYCODE_UNKNOWN; // reset keyCodeOnKeyDown
                    tracker.onDownEvent(lastKey.mX, lastKey.mY, SystemClock.uptimeMillis(), (MainKeyboardView)nCurrentView);
                    tracker.onUpEvent(lastKey.mX, lastKey.mY, SystemClock.uptimeMillis());
                    return true;
    ...
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        ...
        if ((nCurrentView != null)&&(nCurrentView.isShown()&& !((MainKeyboardView)nCurrentView).hasMoreKeysPanel())) {
            if (keyCode==KeyEvent.KEYCODE_ENTER || keyCode==KeyEvent.KEYCODE_DPAD_CENTER) {
                long when = SystemClock.uptimeMillis();
                setFields();
                if (when>mLastKeyUpTime && when<mLastKeyUpTime+INVALID_KEYUP_INTERVAL) { // detect invalid key up event when long press
                    setFields();
                    mSpaceState = SPACE_STATE_NONE;
                    handleBackspace(mSpaceState);
                    mDeleteCount++;
                    mExpectingUpdateSelection = true;
                    LatinImeLogger.logOnDelete(lastKey.mX, lastKey.mY);
                }
                keyCodeOnKeyDown = lastKey.mCode;
                event.startTracking();
                return true;
            }
             // to prevent this keyevent pass to other UI component.
        }

        //Log.d(TAG, "Window's setSoftInputMode ~~");
        ...
        if ((nCurrentView != null)&&(nCurrentView.isShown())) {
            final PointerTracker tracker = PointerTracker.getPointerTracker(0, (MainKeyboardView)nCurrentView);
            if (!isOnKeyLongPress && ((MainKeyboardView)nCurrentView).hasMoreKeysPanel()) { // the MoreKeysKeyboard is showing.
                nKeys = tracker.getKeyboard().mKeys;
                nKeyNums = nKeys.length;

                switch(keyCode) {
                case KeyEvent.KEYCODE_DPAD_DOWN:
                    for (int i=0; i<nKeyNums; i++) {
                        nkey= nKeys[i];
                        if (lastMoreKey==null)
                            lastMoreKey = tracker.getKey();
                        // find the nearest key
                        if (nkey.mX <= lastMoreKey.mX && nkey.mX > lastMoreKey.mX-nkey.mWidth && nkey.mY >= lastMoreKey.mY+lastMoreKey.mHeight && nkey.mY < lastMoreKey.mY+lastMoreKey.mHeight*2) {
                            lastMoreKey = nkey;
                            // move tracker to the center of the lastMoreKey.
                            tracker.onMoveEvent(lastMoreKey.mX+(lastMoreKey.mWidth/2), lastMoreKey.mY+(lastMoreKey.mHeight/2), SystemClock.uptimeMillis(), null);
                            break;
                        }
                    }
                    break;
                case KeyEvent.KEYCODE_DPAD_UP:
                    for (int i=0; i<nKeyNums; i++) {
                        ...
                        if (lastMoreKey==null)
                            lastMoreKey = tracker.getKey();
                        // find the nearest key
                        if (nkey.mX >= lastMoreKey.mX && nkey.mX < lastMoreKey.mX+lastMoreKey.mWidth && nkey.mY <= lastMoreKey.mY-lastMoreKey.mHeight && nkey.mY > lastMoreKey.mY-lastMoreKey.mHeight*2) {
                            lastMoreKey = nkey;
                            // move tracker to the center of the lastMoreKey.
                            tracker.onMoveEvent(lastMoreKey.mX+(lastMoreKey.mWidth/2), lastMoreKey.mY+(lastMoreKey.mHeight/2), SystemClock.uptimeMillis(), null);
                            break;
                ...
                case KeyEvent.KEYCODE_DPAD_LEFT:
                    for (int i=0; i<nKeyNums; i++) {
                        ...
                        if (lastMoreKey==null)
                            lastMoreKey = tracker.getKey();
                        // find the nearest key
                        if (nkey.mY == lastMoreKey.mY && nkey.mX <= lastMoreKey.mX-lastMoreKey.mWidth && nkey.mX > lastMoreKey.mX-lastMoreKey.mWidth*2) {
                            lastMoreKey = nkey;
                            // move tracker to the center of the lastMoreKey.
                            tracker.onMoveEvent(lastMoreKey.mX+(lastMoreKey.mWidth/2), lastMoreKey.mY+(lastMoreKey.mHeight/2), SystemClock.uptimeMillis(), null);
                            break;
                ...
                case KeyEvent.KEYCODE_BACK:
                    nCurrentView.dismissMoreKeysPanel();
                    break;
                }
            }
            else {
                ...
                case KeyEvent.KEYCODE_DPAD_DOWN:
                    ...
                    for (int i = 0; i < nKeyNums; i++) {
                        nkey= nKeys[i];
                        if ((nkey.mY <=  lastKey.mY)||((nkey.mX+nkey.mWidth) < lastKey.mX)||(nkey.mX > (lastKey.mX+lastKey.mWidth))) {
                            continue;
                        }
                        else {
                            nearestKey = nKeys[nearestIndex];
                            if (nearestIndex == nLastKeyIndex) {
                                nearestIndex=i;
                            }
                            else if (nkey.mY < nearestKey.mY) {
                                nearestIndex=i;
                            }
                        }
                    }
                ...
                case KeyEvent.KEYCODE_DPAD_LEFT:
                    ...
                    for (int i = 0; i < nKeyNums; i++) {
                        nkey= nKeys[i];
                        if (nkey.mY != lastKey.mY) {
                            continue;
                        }
                        else if (nkey.mX < lastKey.mX) {
                            nearestKey = nKeys[nearestIndex];
                            if (nearestKey.equals(lastKey)) {
                                nearestIndex=i;
                            }
                            else if (nkey.mX > nearestKey.mX) {
                                nearestIndex=i;
                            }
                        }
                    }
                ...
                case KeyEvent.KEYCODE_NUMPAD_ENTER:
                    performEditorAction(getActionId(mKeyboardSwitcher.getKeyboard()));
                    return true;
                ...
                case KeyEvent.KEYCODE_9:
                	...
                	if (mSuggestionStripView != null && mSuggestionStripView.addToDictionary(index)) {
                		return true;
                	}
                	if (mSuggestionStripView != null && mSuggestionStripView.selectSuggestions(index - 1)) {
                		return true;
                	}
                default:
                    char unicodeChar = (char)event.getUnicodeChar();
                    if (isPrintableChar(unicodeChar)) {
                        onTextInput(String.valueOf(unicodeChar));
                        return true;
                    }
                    return super.onKeyDown(keyCode, event);
                }
                if (nearestIndex != nLastKeyIndex) {
                    nCurrentView.setLastKeyIndex(nearestIndex);
                    nCurrentView.invalidate();
                }
            }
    ...
    @Override
    public void onStartInput(final EditorInfo editorInfo, final boolean restarting) {
        if (isAmazonAPKRunning() && editorInfo.inputType==EditorInfo.TYPE_NULL) {
            handleClose();
        }
        else {
    ...
    private void onStartInputViewInternal(final EditorInfo editorInfo, final boolean restarting) {
        ...
        if (getCurrentInputConnection()!=null && getCurrentInputConnection().getSelectedText(0) == null) {
            editorInfo.initialSelStart = mLastSelectionStart;
            editorInfo.initialSelEnd = mLastSelectionEnd;
        }
    ...
    public boolean isAmazonAPKRunning() {
        return getCurrentInputEditorInfo().packageName.contains("com.amazon");
    }
    @Override
    public boolean onEvaluateFullscreenMode() {
        // Reread resource value here, because this method is called by framework anytime as needed.
        final boolean isFullscreenModeAllowed =
                mCurrentSettings.isFullscreenModeAllowed(getResources()) && !isAmazonAPKRunning();
    ...
    public void pickSuggestionManually(final int index, final CharSequence suggestion) {
		...
		boolean canAddToDirectory = false;
        if (showingAddToDictionaryHint && mIsUserDictionaryAvailable && canAddToDirectory) {
    ...
    public void onReleaseKey(final int primaryCode, final boolean withSliding) {
        ...
        if (withSliding==false) {
            nCurrentView = mKeyboardSwitcher.getMainKeyboardView();
            setFields();
            for (int i = 0; i < nKeyNums; i++) {
                nkey = nKeys[i];
                if (primaryCode == nkey.mCode) {
                    nCurrentView.setLastKeyIndex(i);
                    nCurrentView.invalidate();
                    break;
                }
            }
