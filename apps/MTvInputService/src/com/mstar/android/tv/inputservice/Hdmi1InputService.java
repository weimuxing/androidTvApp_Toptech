 ...
 * Hdmi1InputService
 */
public class Hdmi1InputService extends HdmiInputService {
    private static final String TAG = "Hdmi1InputService";
    ...
    public Session onCreateSession(String inputId) {
        ...
        if (WindowControl.acquireWindow(inputId)) {
            return new Hdmi1SessionImpl(this, inputId);
        } else {
            return null;
        }
    }
    ...
    public TvInputInfo onHardwareAdded(TvInputHardwareInfo hardwareInfo) {
        if (hardwareInfo.getType() != TvInputHardwareInfo.TV_INPUT_TYPE_HDMI ||
                hardwareInfo.getHdmiPortId() != HdmiInputService.PORT1) {
            return null;
        }
        ComponentName name = new ComponentName(this, Hdmi1InputService.class);
    ...
    public TvInputInfo onHdmiDeviceAdded(HdmiDeviceInfo deviceInfo) {
        if (mHardwareInputInfoId == null ||
                deviceInfo.getPortId() != HdmiInputService.PORT1) {
            return null;
        }
        ComponentName name = new ComponentName(this, Hdmi1InputService.class);
        ResolveInfo resInfo = getResolveInfo(name.getClassName());
    ...
    static class Hdmi1SessionImpl extends HdmiSessionImpl {
        Hdmi1SessionImpl(Context context, String inputId) {
            super(context, inputId);
            Log.d(TAG, "Hdmi1SessionImpl");
            if (mDeviceId == TvCommonManager.INPUT_SOURCE_NONE) {
                mDeviceId = TvCommonManager.INPUT_SOURCE_HDMI;
            }
        }
    }
}
