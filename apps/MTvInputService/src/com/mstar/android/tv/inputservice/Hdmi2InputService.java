 ...
 * Hdmi2InputService
 */
public class Hdmi2InputService extends HdmiInputService {
    private static final String TAG = "Hdmi2InputService";
    ...
    public Session onCreateSession(String inputId) {
        ...
        if (WindowControl.acquireWindow(inputId)) {
            return new Hdmi2SessionImpl(this, inputId);
        } else {
            return null;
        }
    }
    ...
    public TvInputInfo onHardwareAdded(TvInputHardwareInfo hardwareInfo) {
        if (hardwareInfo.getType() != TvInputHardwareInfo.TV_INPUT_TYPE_HDMI ||
                hardwareInfo.getHdmiPortId() != HdmiInputService.PORT2) {
            return null;
        }
        ComponentName name = new ComponentName(this, Hdmi2InputService.class);
    ...
    public TvInputInfo onHdmiDeviceAdded(HdmiDeviceInfo deviceInfo) {
        if (mHardwareInputInfoId == null ||
                deviceInfo.getPortId() != HdmiInputService.PORT2) {
            return null;
        }
        ComponentName name = new ComponentName(this, Hdmi2InputService.class);
        ResolveInfo resInfo = getResolveInfo(name.getClassName());
    ...
    static class Hdmi2SessionImpl extends HdmiSessionImpl {
        private int mPortId = -1;

        Hdmi2SessionImpl(Context context, String inputId) {
            super(context, inputId);
            Log.d(TAG, "Hdmi2SessionImpl");
            if (mDeviceId == TvCommonManager.INPUT_SOURCE_NONE) {
                mDeviceId = TvCommonManager.INPUT_SOURCE_HDMI2;
            }
        }
    }
}
