 ...
 * Hdmi3InputService
 */
public class Hdmi3InputService extends HdmiInputService {
    private static final String TAG = "Hdmi3InputService";
    ...
    public Session onCreateSession(String inputId) {
        ...
        if (WindowControl.acquireWindow(inputId)) {
            return new Hdmi3SessionImpl(this, inputId);
        } else {
            return null;
        }
    }
    ...
    public TvInputInfo onHardwareAdded(TvInputHardwareInfo hardwareInfo) {
        if (hardwareInfo.getType() != TvInputHardwareInfo.TV_INPUT_TYPE_HDMI ||
                hardwareInfo.getHdmiPortId() != HdmiInputService.PORT3) {
            return null;
        }
        ComponentName name = new ComponentName(this, Hdmi3InputService.class);
    ...
    public TvInputInfo onHdmiDeviceAdded(HdmiDeviceInfo deviceInfo) {
        if (mHardwareInputInfoId == null ||
                deviceInfo.getPortId() != HdmiInputService.PORT3) {
            return null;
        }
        ComponentName name = new ComponentName(this, Hdmi3InputService.class);
        ResolveInfo resInfo = getResolveInfo(name.getClassName());
    ...
    static class Hdmi3SessionImpl extends HdmiSessionImpl {
        private int mPortId = -1;

        Hdmi3SessionImpl(Context context, String inputId) {
            super(context, inputId);
            Log.d(TAG, "Hdmi3SessionImpl");
            if (mDeviceId == TvCommonManager.INPUT_SOURCE_NONE) {
                mDeviceId = TvCommonManager.INPUT_SOURCE_HDMI3;
            }
        }
    }
}
