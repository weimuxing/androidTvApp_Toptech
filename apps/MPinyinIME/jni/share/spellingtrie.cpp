...
bool SpellingTrie::save_spl_trie(FILE *fp) {
  ...
  if (fwrite(&spelling_size_, sizeof(int32_t), 1, fp) != 1)
    return false;

  if (fwrite(&spelling_num_, sizeof(int32_t), 1, fp) != 1)
    return false;
...
bool SpellingTrie::load_spl_trie(FILE *fp) {
  ...
  // MStar Android Patch Begin
  if (fread(&spelling_size_, sizeof(int32_t), 1, fp) != 1)
    return false;

  if (fread(&spelling_num_, sizeof(int32_t), 1, fp) != 1)
    return false;
  // MStar Android Patch End
