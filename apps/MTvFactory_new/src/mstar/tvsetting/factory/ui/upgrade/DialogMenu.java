...
public class DialogMenu extends Dialog {
	...
	private static String MSTARUPGRADE;
	...
	private static final String[] upgradeStatus = { "Fail!", "Success!", "File Not Found!", "Please Plug USB!" };
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		try {
			MSTARUPGRADE = TvManager.getInstance().getEnvironment("ForceUpgradePath");
		} catch (TvCommonException e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
		if (parentItemId == R.id.textview_upgrade_mboot2) {
			m_dialog_upgrade_type = DIALOG_UPGRADE_MBOOT;
			setContentView(R.layout.dialogmenu);
		}
		if (parentItemId == R.id.textview_upgrade_main2) {
			m_dialog_upgrade_type = DIALOG_UPGRADE_MAIN;
			setContentView(R.layout.dialogmenu);
		}
		if (parentItemId == R.id.textview_upgrade_6m30_2) {
			m_dialog_upgrade_type = DIALOG_UPGRADE_6M30;
			setContentView(R.layout.dialogmenu);
		}
		if (parentItemId == R.id.textview_upgrade_dual_ursa2) {
		...
		if ("" == mainpath) {
			...
			try {
				if (TvManager.getInstance().setEnvironment("upgrade_mode", "usb")
						&& TvManager.getInstance().setEnvironment("MstarUpgrade_complete", "0")) {
	...
	public void DialogMenuConfirm(int which) {
		switch (which) {
		case R.id.textview_upgrade_mboot2:
			...
			break;
		case R.id.textview_upgrade_main2:
			if (CheckUsbIsExist() == true) {
			...
			break;
		case R.id.textview_upgrade_6m30_2:
			if (CheckUsbIsExist() == true) {
			...
			break;
		case R.id.textview_upgrade_dual_ursa2:
			if (CheckUsbIsExist() == true) {
	...
	private static ProgressDialog getProgressDialog() {
		if (progressDialog == null) {
			...
			progressDialog
					.setMessage(dialogmenuactivity.getString(R.string.str_textview_factory_otheroption_waiting_val));
