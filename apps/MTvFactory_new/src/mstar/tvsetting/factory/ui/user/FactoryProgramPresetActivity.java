...
public class FactoryProgramPresetActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		String[] dtvRouteTypeStr = getResources().getStringArray(R.array.str_arr_dtv_route_type);
		mFactoryProgramFinishedEventListener = new factoryProgramFinishedEventListener();
		TvFactoryManager.getInstance()
				.registerOnFactoryProgramFinishedEventListener(mFactoryProgramFinishedEventListener);
		...
		if (TvChannelManager.TV_ROUTE_NONE == dtvRouteTypeVal) {
			Toast.makeText(FactoryProgramPresetActivity.this,
					this.getResources().getString(R.string.str_factoryprogrampreset_no_support_dtv_route),
			...
			final String[] arr = (String[]) mSupportDtvRouteType.toArray(new String[mSupportDtvRouteType.size()]);
			mAtvPreset.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					mProgressDialogAtv = ProgressDialog.show(FactoryProgramPresetActivity.this,
							FactoryProgramPresetActivity.this.getResources()
									.getString(R.string.str_factoryprogrampreset_waiting),
							FactoryProgramPresetActivity.this.getResources().getString(
									R.string.str_factoryprogrampreset_being_preseted_atv),
			...
			mDtvPreset.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					...
					new AlertDialog.Builder(FactoryProgramPresetActivity.this)
							.setTitle(FactoryProgramPresetActivity.this.getResources()
									.getString(R.string.str_factoryprogrampreset_choose_type))
							.setItems(arr, new DialogInterface.OnClickListener() {
								@Override
								public void onClick(DialogInterface dialog, int which) {
									TvFactoryManager.getInstance().restoreFactoryDtvProgramTableByRoute(
											TvFactoryManager.CITY_NUM_ONE,
											mSupportDtvRouteTypeData.get(which).intValue());
									mProgressDialogDtv = ProgressDialog.show(FactoryProgramPresetActivity.this,
											FactoryProgramPresetActivity.this.getResources()
													.getString(R.string.str_factoryprogrampreset_waiting),
											FactoryProgramPresetActivity.this.getResources()
													.getString(R.string.str_factoryprogrampreset_being_preseted_dtv),
											true, true, mCancelListener);
								}
							})
							.setNegativeButton(
									FactoryProgramPresetActivity.this.getResources()
											.getString(R.string.str_factoryprogrampreset_cancel),
	...
	protected void onDestroy() {
		Log.i(TAG, "onDestroy");
		TvFactoryManager.getInstance()
				.unregisterOnFactoryProgramFinishedEventListener(mFactoryProgramFinishedEventListener);
	...
	private class factoryProgramFinishedEventListener
			implements TvFactoryManager.OnFactoryProgramFinishedEventListener {
		@Override
	...
	public void findView() {
		...
		mLayoutProgramPresetAtvOrDtv = (LinearLayout) findViewById(
				R.id.linearlayout_factory_otheroption_factoryprogrampreset_atv_dtv);
		...
		case KeyEvent.KEYCODE_ENTER:
			...
			case R.id.linearlayout_factory_otheroption_factoryprogramreset:
				if (mFactoryManager.resetToFactoryDefault() == true) {
					mTextProgramResetState.setText(R.string.str_textview_factory_otheroption_text_pass);
				} else {
					mTextProgramResetState.setText(R.string.str_textview_factory_otheroption_text_fail);
				}
				break;
			case R.id.linearlayout_factory_otheroption_backupdbtousb:
				if (BackupDBtoUSB() == 0) {
					mTextBackupDbtoUsbState.setText(R.string.str_textview_factory_otheroption_text_pass);
				} else {
					mTextBackupDbtoUsbState.setText(R.string.str_textview_factory_otheroption_text_fail);
				}
				break;
			case R.id.linearlayout_factory_otheroption_restoredbfromusb:
				if (RestoreDBfromUSB() == 0) {
					mTextRestoreDbfromUsbState.setText(R.string.str_textview_factory_otheroption_text_pass);
				} else {
					mTextRestoreDbfromUsbState.setText(R.string.str_textview_factory_otheroption_text_fail);
				}
				break;
			default:
				break;
	...
	private int BackupDBtoUSB() {
		...
		String[] volumes = getPaths();

		if (volumes == null) {
			Toast.makeText(this, this.getResources().getString(R.string.str_USB_not_exist), Toast.LENGTH_SHORT).show();
			return ret;
		}
	...
	public static String[] getPaths() {
		File rootFile = new File("/mnt/usb");
		if (rootFile == null || !rootFile.exists()) {
			System.out.println(rootFile.getAbsolutePath() + " not exist!");
		} else {
