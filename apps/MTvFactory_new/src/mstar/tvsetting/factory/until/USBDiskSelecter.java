...
public abstract class USBDiskSelecter {
	...
	public void start() {
		...
		al.setOnKeyListener(new OnKeyListener() {
	...
	private void updateUSBDriverInfo() {
		...
		for (int i = 0; i < volumes.length; ++i) {
			...
			if (volumeLabel != null) {
				...
				Log.d(TAG, "volumeLabel = " + "Removable Disk");
				volumeLabel = context.getString(R.string.str_removable_disk);
			}
	...
	private View getUSBSelecterView(int usbCount) {
		...
		if (usbCount <= maxDiskperPage) {
			...
			LayoutParams gridparams = new LayoutParams((dip2px(120 + 5)) * usbCount, LayoutParams.WRAP_CONTENT);
		...
		usbListView.setOnItemSelectedListener(
				new usbDiskSelectedListener((HorizontalScrollView) view.findViewById(R.id.usb_driver_scroller)));
	...
	private boolean getChooseDiskSettings() {
		...
		SharedPreferences sp = context.getSharedPreferences(Constant.SAVE_SETTING_SELECT, Context.MODE_PRIVATE);
		...
		if (file.isDirectory()) {
			...
			for (File tmp : list) {
				if (isFileExisted(tmp.getAbsolutePath() + "/_MSTPVR/")
						|| mstar.tvsetting.factory.until.USBBroadcastReceiver
	...
	public String getBestDiskPath() {
		if (getChooseDiskSettings()) {
			...
			if (isFileExisted(path + "/_MSTPVR")
					|| mstar.tvsetting.factory.until.USBBroadcastReceiver.isDiskExisted(path)) {
				return path;
			} else {
