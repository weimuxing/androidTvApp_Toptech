...
import com.jrm.localmm.business.data.ReturnData;
import com.jrm.localmm.business.data.ReturnStack;
...
public class LocalDataManager extends BaseDataManager {
	...
	private List<BaseData> typeList = new ArrayList<BaseData>();
    ...
    // Status flag
    private int state = STATUS_DEVICE_DISPLAY;
    ...
    private FileBrowserActivity fileBrowserActivity =new FileBrowserActivity();
     ...
     */
    protected void getCurrentPage(int page, int type,int pos) {
	...
	protected void getCurrentPage(int page, int type) {
		getCurrentPage(page,type,-1);
	}
	
	protected void findFocus(String name){
		int a =0;
		int b =0;
		for(int i=0;i<typeList.size();i++){
			String n=typeList.get(i).getName();
			if(n.equals(name)){
					a=(i/9+1);
					b=(i%9+1);
			}
		...
		if(a<curpage){
			for(int k=0;k<curpage-a;k++){
				getCurrentPage(-1,0,b);
			}	
		}else if(a>curpage){
			for(int j=0;j<a-curpage;j++){
			getCurrentPage(1,0,b);
			}	
		}else{
		    // change by jerry 20160831
			getCurrentPage(0,0,b);
			//getCurrentPage(1,0,b);
		}
	}
	...
	/*new Handler().postDelayed(new Runnable(){
		@Override
		public void run() {
			refreshUIListener.onFinish(uiData, 1, totalPage, b);
		}},2500);*/
    ...
    protected int getMediaFile(int type, int position) {
        ...
        int index=-100;
        if (Constants.GRIDVIEW_MODE== FileBrowserActivity.mListOrGridFlag) {
            index= (currentPage - 1) * Constants.GRID_MODE_DISPLAY_NUM+ position - 1;
        } else if (Constants.LISTVIEW_MODE== FileBrowserActivity.mListOrGridFlag) {
            index = (currentPage - 1) * Constants.LIST_MODE_DISPLAY_NUM+ position - 1;
        }
    ...
    private void enterParentDirectory() {
        ...
        if (returnStack.getTankage() > 0) {
            ...
            if (currentDirectory != null
                    && currentDirectory.equals(Tools.getUSBMountedPath())) {
                showUSBDevice();
                Constants.ExitMM = true;
    ...
    private void enterDirectory(int index) {
        ...
        if (state == STATUS_DEVICE_DISPLAY) {
            ...
            if (Constants.GRIDVIEW_MODE== FileBrowserActivity.mListOrGridFlag) {
                tmp = (currentPage - 1) * Constants.GRID_MODE_DISPLAY_NUM + index- 1;
            } else if (Constants.LISTVIEW_MODE== FileBrowserActivity.mListOrGridFlag) {
                tmp = (currentPage - 1) * Constants.LIST_MODE_DISPLAY_NUM + index- 1;
            }
        ...
        for (StorageVolume item : volumes) {
            ...
            if (state == null || !state.equals(Environment.MEDIA_MOUNTED)) {
               ...
               // if(SystemProperties.getBoolean("mstar.cus.onida", false)){
								if(name.equals("0"))
								   name=activity.getResources().getString(R.string.str_internal_storage);
								else if(name.equals("ext_sdcard"))
								   name=activity.getResources().getString(R.string.str_external_sdcard);
