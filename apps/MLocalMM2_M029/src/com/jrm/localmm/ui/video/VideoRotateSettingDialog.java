...
public class VideoRotateSettingDialog extends Dialog {
    ...
    private void setVideoRotateDegrees(int position) {
        ...
        if (!mVideoRotateDegresArray.get(position).equals(rotateDegrees)) {
            ...
            boolean status = mContext.imageRotate(mContext.getVideoPlayHolder().getViewId(), Tools.getRotateDegrees());
            Log.i(TAG, "rotate status:" + status);
            // this.dismiss();
            // Toast.makeText(mContext, mContext.getResources().getString(R.string.video_rotate_degrees_setting_info), Toast.LENGTH_SHORT).show();
        }
    }
