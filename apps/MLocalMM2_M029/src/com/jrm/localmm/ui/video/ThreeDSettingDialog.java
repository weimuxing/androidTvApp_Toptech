...
public class ThreeDSettingDialog extends Dialog {
        ...
        if (mIsSTB) {
            ...
            if (!Tools.unSupportTVApi()) {
                if (context.getString(R.string.video_3D_3dto2d).equals(opt)){
                    ...
                    for (int i = 0; i < size; i++) {
                         if (context.displayFormat == threeDFormat.get(i)) {
                             selectPosition = i;
                             video3DSettingList.setSelection(i);
                             break;
                         }
            ...
            if (!Tools.unSupportTVApi()) {
                for (int i = 0; i < size; i++) {
                    if (this.context.displayFormat == threeDFormat.get(i)) {
                        selectPosition = i;
                        video3DSettingList.setSelection(i);
                         break;
                     }
    ...
    private void set3DMode(int position) {
        ...
        TvS3DManager s3dSkin = TvS3DManager.getInstance();
        EnumThreeDVideo3DTo2D twoDformat = null;
        if (mIsSTB) {
            if (Tools.is3DTVPlugedIn()) {
                String opt = Tools.getPlaySettingOpt(0, mViewID);
                if (context.getString(R.string.video_3D_3dto2d).equals(opt)) {
                    twoDformat = EnumThreeDVideo3DTo2D.values()[position+1];
                    Log.i(TAG, "set3DTo2D twoDformat:" + twoDformat);
                    s3dSkin.set3DTo2D(twoDformat);
                } else {
                    context.displayFormat = (EnumThreeDVideoDisplayFormat) threeDFormat.get(position);
                    Log.i(TAG, "setDisplayFormat context.displayFormat:" + context.displayFormat);
