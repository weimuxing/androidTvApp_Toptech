...
public class ImagePlayerThreeDSetting extends Dialog {
    ...
    private ArrayList<Object> threeDFormat = new ArrayList<Object>();
    ...
    private void initView() {
        ...
        if (!Tools.isMstarSTB()) {
            ...
            if (!Tools.unSupportTVApi()) {
                threeDFormat.add(EnumThreeDVideoDisplayFormat.E_ThreeD_Video_DISPLAYFORMAT_NONE);
                threeDFormat.add(EnumThreeDVideoDisplayFormat.E_ThreeD_Video_DISPLAYFORMAT_SIDE_BY_SIDE);
                threeDFormat.add(EnumThreeDVideoDisplayFormat.E_ThreeD_Video_DISPLAYFORMAT_TOP_BOTTOM);
                threeDFormat.add(EnumThreeDVideoDisplayFormat.E_ThreeD_Video_DISPLAYFORMAT_FRAME_PACKING);
                threeDFormat.add(EnumThreeDVideoDisplayFormat.E_ThreeD_Video_DISPLAYFORMAT_LINE_ALTERNATIVE);
                threeDFormat.add(EnumThreeDVideoDisplayFormat.E_ThreeD_Video_DISPLAYFORMAT_2DTO3D);
                // Currently some photo do 3D auto detect will be failed, so Comment out it.Mantis:640550
                // threeDFormat.add(EnumThreeDVideoDisplayFormat.E_ThreeD_Video_DISPLAYFORMAT_AUTO);
                threeDFormat.add(EnumThreeDVideoDisplayFormat.E_ThreeD_Video_DISPLAYFORMAT_CHECK_BOARD);
                threeDFormat.add(EnumThreeDVideoDisplayFormat.E_ThreeD_Video_DISPLAYFORMAT_PIXEL_ALTERNATIVE);
                threeDFormat.add(EnumThreeDVideoDisplayFormat.E_ThreeD_Video_DISPLAYFORMAT_FRAME_ALTERNATIVE);
            }
        } else {
            // STB
            mItemName.add(this.mContext.getResources().getString(R.string.video_3D_none));
            ...
            if (!Tools.unSupportTVApi()) {
                ...
                if (is3DTVPlugedIn) {
                    ...
                    // 2D TV
                    threeDFormat.add(EnumThreeDVideo3DTo2D.E_ThreeD_Video_3DTO2D_NONE);
                    threeDFormat.add(EnumThreeDVideo3DTo2D.E_ThreeD_Video_3DTO2D_SIDE_BY_SIDE);
                    threeDFormat.add(EnumThreeDVideo3DTo2D.E_ThreeD_Video_3DTO2D_TOP_BOTTOM);
                    threeDFormat.add(EnumThreeDVideo3DTo2D.E_ThreeD_Video_3DTO2D_FRAME_PACKING);
                    threeDFormat.add(EnumThreeDVideo3DTo2D.E_ThreeD_Video_3DTO2D_LINE_ALTERNATIVE);
                }
        ...
        if (!Tools.unSupportTVApi()) {
            ...
            for (int i = 0; i < size; i++) {
                if (Tools.isMstarSTB()) {
                    ...
                    if (is3DTVPlugedIn) {
                        // 3D TV
                        if (this.mContext.displayFormat == (EnumThreeDVideoDisplayFormat)threeDFormat.get(i)) {
                            mSelectedPosition = i;
                        }
                    } else {
                        // 2D TV
                        if (this.mContext.mDisplayMode3DTo2D == (EnumThreeDVideo3DTo2D)threeDFormat.get(i)) {
                            mSelectedPosition = i;
                        }
                    }
                } else {
    ...
    private void  refreshThreeDSettingDialogItemView() {
        if (!Tools.unSupportTVApi()) {
            new Thread(new Runnable() {
                ...
                public void run() {
                    if (Tools.isMstarSTB()) {
                        ...
                        if (is3DTVPlugedIn) {
                            // 3D TV
                            mContext.displayFormat = Tools.getCurrent3dFormat();
                            // Currently some photo do 3D auto detect will be failed, so Comment out it.Mantis:640550
                            if (mContext.displayFormat == EnumThreeDVideoDisplayFormat.E_ThreeD_Video_DISPLAYFORMAT_AUTO) {
                                mContext.displayFormat = EnumThreeDVideoDisplayFormat.E_ThreeD_Video_DISPLAYFORMAT_NONE;
                            }
                        } else {
                            // 2D TV
                            mContext.mDisplayMode3DTo2D = Tools.getCurrent3dFormatOnSTB2DTV();
                        }
                    } else {
                        mContext.displayFormat = Tools.getCurrent3dFormat();
                        // Currently some photo do 3D auto detect will be failed, so Comment out it.Mantis:640550
                    ...
                    for (int i = 0; i < size; i++) {
                        if (Tools.isMstarSTB()) {
                            ...
                            if (is3DTVPlugedIn) {
                                // 3D TV
                                if (mContext.displayFormat == (EnumThreeDVideoDisplayFormat)threeDFormat.get(i)) {
                                    mSelectedPosition = i;
                                }
                            } else {
                                // 2D TV
                                if (mContext.mDisplayMode3DTo2D == (EnumThreeDVideo3DTo2D)threeDFormat.get(i)) {
                                    mSelectedPosition = i;
                                }
                            }
                        } else {
    ...
    private void set3DMode(int position) {
        ...
        if (Tools.isMstarSTB()) {
            ...
            if (is3DTVPlugedIn) {
                // 3D TV
                this.mContext.displayFormat = (EnumThreeDVideoDisplayFormat) threeDFormat.get(position);
            } else {
                // 2D TV
                this.mContext.mDisplayMode3DTo2D = (EnumThreeDVideo3DTo2D) threeDFormat.get(position);
            }
            Tools.setVideoMute(true, 50);
            if (is3DTVPlugedIn) {
                // 3D TV
                Tools.setDisplayFormat(this.mContext.displayFormat);
            } else {
                // 2D TV
                Tools.set3DTo2D(this.mContext.mDisplayMode3DTo2D);
            }
