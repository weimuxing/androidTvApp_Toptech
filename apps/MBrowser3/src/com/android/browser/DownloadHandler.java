...
public class DownloadHandler {
    ...
    private static final boolean LOGD_ENABLED =
            com.android.browser.MBrowser.LOGD_ENABLED;
    ...
    public static void onDownloadStart(Activity activity, String url,
        ...
        Log.i(LOGTAG, " onDownloadStart url:"+url+"   mimetype:"+mimetype+"  contentDisposition: "+contentDisposition);
    ...
    // MStar Android Patch Begin
    // Parse Base64 URL
    static String parseBase64URL (String sURL) {
        String sPart0[] = sURL.split("\\/");
        String sPart1 = sPart0[sPart0.length-1];
        if (sPart1.length() > 4) {
            String sReal = sPart1.substring(4);
            if (sPart1.substring(0,4).equals("COFF")) {
                String sBase64[] = sReal.split("\\.");
                byte[] bytes = Base64.decode(sBase64[0],Base64.DEFAULT);
                try {
                    String sEntire = new String(bytes,"gb2312");
                    String sPlit[] = sEntire.split("&");
                    String filename = sPlit[5].substring(2) + "." + sBase64[1];
                    return filename;
                } catch (Exception e) {
                    return "";
                }
    ...
    /*package */ static void onDownloadStartNoStream(Activity activity,
        ...
        // MStar Android Patch Begin
        String realname = parseBase64URL(url);
