...
class TabControl {
   ...
   /**
      * When system memory is so low that it needs to kill
      * browser app,We must force to free other tabs's memory.
      */
   void forceFreeMemory() {
       if (getTabCount() < 2) return;
       for (Tab t : mTabs) {
            if (t != getCurrentTab()) {
    ...
    void freeMemory() {
        ...
        if (tabs.size() > 0) {
            ...
            for (Tab t : tabs) {
                final WebView webview = t.getWebView();
                if (webview != null)
                    webview.freeMemoryForTests();

                final WebView subview = t.getSubWebView();
                if (subview != null)
                    subview.freeMemoryForTests();
