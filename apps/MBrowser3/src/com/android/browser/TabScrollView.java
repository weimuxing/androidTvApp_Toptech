...
public class TabScrollView extends HorizontalScrollView {
    ...
    void setSelectedTab(int position) {
        ...
        if (BrowserSettings.getInstance().enableVirtualMouse()) {
            letPrevOrNextScrollIn();
        }
    }

    Handler mHandler = new Handler();
    private void letPrevOrNextScrollIn() {
        new Thread(){
            public void run() {
                ...
                while(true) {
                    ...
                    if ((currTime - mLastScrollTime) > 250) {
                        break;
                    } else {
                        try {
                            Thread.sleep(10);
    ...
    private void scrollPrevOrNextIn() {
        mHandler.post(new Runnable(){
            ...
            public void run() {
                ...
                int scrollWidth = (int)getWidth();
                int stepX = left - currX;
                // android.util.Log.d("terry","left="+left+" right="+right+" currX="+currX+
                // " tabWidth="+tabWidth+" scrollWidth="+scrollWidth+" stepX="+stepX);
                if (stepX < 0) {
                    int tempX = (currX - left) + tabWidth;
                    setScroll(currX - tempX);
                } else if (stepX < (tabWidth * 0.5)) {
                    int tempX = tabWidth - (left - currX);
                    setScroll(currX - tempX);
                } else if ((stepX + tabWidth) >= (scrollWidth - tabWidth * 0.2)) {
                    setScroll(currX + tabWidth);
                }
    ...
    private long mLastScrollTime = System.currentTimeMillis();
    @Override
