...
public class Controller
    ...
    public final static int OPEN_NEW_TAB = 1003;
    public final static int CLOSE_CURRENT_TAB = 1004;
    ...
    private static final int START_XML_FILE = 204;
    ...
    //Ref:http://www.jcodecraeer.com/a/anzhuokaifa/androidkaifa/2015/0830/3387.html
    //https://github.com/mengdd/AndroidMRuntimePermissionSample
    final static int PERMISSIONS_WRITE_EXTERNAL_STORAGE = 123;
    ...
    private volatile boolean isTestThreadShouldStop = false;
        ...
        mHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                switch (msg.what) {
                        ...
                        switch (msg.arg1) {
                            ...
                            case R.id.download_context_menu_id:
                                ...
                                DownloadHandler.onDownloadStartNoStream(
                                        ...
                                        null, null, refererUrl, view.isPrivateBrowsingEnabled());
                                break;
                        }
                        break;
                    }
                    ...
                    case OPEN_NEW_TAB:
                        openTab((String) msg.obj, false, true, false);
                        break;
                    ...
                    case START_DOWNLOAD_FILE:
                        {
                            ...
                            doDownloading(downloadinfo.tab, downloadinfo.url, downloadinfo.userAgent, downloadinfo.contentDisposition, downloadinfo.mimetype,downloadinfo.RefererUrl, downloadinfo.contentLength);
                        }
                        break;
                    // Mstar Android Patch Begin
                    case START_PLAY_FILE:
                        {
                            DownloadInfo  downloadinfo = (DownloadInfo)msg.obj;
                            doPlayFile(downloadinfo.tab, downloadinfo.url, downloadinfo.userAgent, downloadinfo.contentDisposition, downloadinfo.mimetype,downloadinfo.RefererUrl, downloadinfo.contentLength);
                        }
                        break;
                    case START_XML_FILE:
                        {
                            DownloadInfo  downloadinfo = (DownloadInfo)msg.obj;
                            doParseXmlFile(downloadinfo.tab, downloadinfo.url, downloadinfo.userAgent, downloadinfo.contentDisposition, downloadinfo.mimetype,downloadinfo.RefererUrl, downloadinfo.contentLength);
                        }
                        break;
                    // Mstar Android Patch End
                }
    ...
    public void onStop() {
        ...
        if (mUi.isCustomViewShowing()) {
            Log.i(LOGTAG,"controller onStop to hide Custom View");
            hideCustomView();
        }
        // pause tab from onPause to onStop, because play inline video will stop when press mix or change source key
        // mantis:1011884 by fish.xu
        pauseTab();
        mMouseControl.closeInputDevice();
    }

    @Override
    public void onForceFreeMemory() {
        mTabControl.forceFreeMemory();
    }

    private void pauseTab() {
        if (mActivityPaused) {
    ...
    public void onPause() {
        ...
        if (BrowserActivity.mPIPOpen && mUi.isCustomViewShowing())
            return;
        if (mUi.isCustomViewShowing()) {
            hideCustomView();
        }
    ...
    public void onResume() {
        ...
        mUi.onResume();
        if (!BrowserSettings.getInstance().enableTestMode()) {
            mUi.setLoadSpeedTextViewVisible(false);
        }
    ...
    @Override
    public void onDownloadStart(Tab tab, String url, String userAgent, String contentDisposition, String mimetype, String referer,
            long contentLength) {
            Log.i(LOGTAG, " onDownloadStart  url:"+url+"  mimmetype:"+mimetype+"  contentDisposition:"+contentDisposition);
            DownloadInfo downloadinfo = new DownloadInfo();
            downloadinfo.tab = tab;
            downloadinfo.RefererUrl = referer;
            ...
            /*
            if (url.toLowerCase().endsWith("mstar_private_test_browser.xml")) {
                Log.i(LOGTAG, "Mstar Private Test XML!");
                mHandler.sendMessage(mHandler.obtainMessage(START_XML_FILE, 0, 0, downloadinfo));
                return;
            }
            */
    ...
    private void doDownloading(Tab tab, String url, String userAgent,
    ...
    private void doPlayFile(Tab tab, String url, String userAgent,
            String contentDisposition, String mimetype, String referer,
            long contentLength) {
        WebView w = tab.getWebView();
        ...
        Log.i(LOGTAG, "--->doPlayFile :"+url);
        w.loadUrl(videoscript);
    }

    private void doParseXmlFile(Tab tab, String url, String userAgent,
            String contentDisposition, String mimetype, String referer,
            long contentLength) {
        WebView w = tab.getWebView();
        Log.i(LOGTAG, "--->doParseXmlFile :"+url);
        final String xmlurl = url;
        isTestThreadShouldStop = false;
         ...
         new Thread(new Runnable() {
            ...
            public void run() {
                ...
                myTest.parse(xmlurl);
                //myTest.dump();
                Log.i(LOGTAG, "test item count:"+myTest.getItemSize());
                for (int i=0; i < myTest.getItemSize(); i++) {
                    if (isTestThreadShouldStop) {
                        Log.i(LOGTAG, "Test Suspend.   load url count:"+i);
                        break;
                    }
                    Test.TestItem  theItem = myTest.getItem(i);
                    if (theItem != null) {
                        Log.i(LOGTAG, "to load url: <"+i+">"+theItem.url +"   tabTarget:"+theItem.tabTarget);
                        if (theItem.tabTarget.equals("open")) {
                            mHandler.sendMessage(mHandler.obtainMessage(OPEN_NEW_TAB, 0, 0, theItem.url));
                        }
                        else if (theItem.tabTarget.equals("close")) {
                            mHandler.sendMessage(mHandler.obtainMessage(CLOSE_CURRENT_TAB, 0, 0, theItem.url));
                            try {
                                Thread.sleep(1000);
                            ...
                            if (!isTestThreadShouldStop) {
                                mHandler.sendMessage(mHandler.obtainMessage(LOAD_URL, 0, 0, theItem.url));
                            }
                        }
                        else
                             mHandler.sendMessage(mHandler.obtainMessage(LOAD_URL, 0, 0, theItem.url));
                        ...
                        try {
                            Thread.sleep(theItem.stayTime);
                        } catch (InterruptedException e) {
                            e.printStackTrace();
                        }
                    }
                }
                Log.i(LOGTAG, "Test  done.  load url count:"+myTest.getItemSize());
            }
        }).start();
    }
    ...
    public void showCustomView(Tab tab, View view, int requestedOrientation,
        ...
        mActivity.sendBroadcast(new Intent("browser.play.video"));
        // Mstar Android Patch Begin
        SystemProperties.set("mstar.browser.video.start","true");
        Log.i(LOGTAG,"mstar.browser.video.start true");
        // Mstar Android Patch End
        mUrlInput = (UrlInputView)getActivity().findViewById(R.id.url);
        mUrlInput.setFocusableInTouchMode(false);
    ...
    public void hideCustomView() {
        // Mstar Android Patch Begin
        // Don't show main view after receive pip fullscreen broadcast..
        // add by liao.fan
        if (BrowserActivity.mPipEnterFullScreen) {
            return;
        }
        // Mstar Android Patch End

        mActivity.sendBroadcast(new Intent("browser.out.video"));
        // Mstar Android Patch Begin
        SystemProperties.set("mstar.browser.video.start","false");
        Log.i(LOGTAG,"mstar.browser.video.start false");
        // Mstar Android Patch End
        if (mUi.isCustomViewShowing()) {
        ...
        if(mUrlInput != null){
            mUrlInput.setFocusableInTouchMode(true);
            mUrlInput.setFocusable (true);
            String title=getCurrentTab().getUrl();
            ...
            if (event.getAction() == KeyEvent.ACTION_DOWN) {
                ...
                switch (event.getKeyCode()) {
                    case KeyEvent.KEYCODE_DPAD_UP:
                        if (mCurrentCursorY <= tabbar.getHeight() + 10)  {
                            browserWebView.scrollBy(0,-SCROLL_SIZE);
                            return true;
                        }
    ...
    private static class Download implements OnMenuItemClickListener {
        ...
        public boolean onMenuItemClick(MenuItem item) {
            ...
            if (mActivity.checkSelfPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE)
                ...
                Log.v(LOGTAG,"requestPermissions");
    ...
    private boolean ShowConfirm(int titleId,String message, final Object obj, boolean  isVideo) {
        Log.i(LOGTAG, "ShowConfirm  isvideo:"+isVideo);
	...
	String sUrl = mSettings.getHomePage();
	//if(mAutoTest)
	//	sUrl = "file:///android_asset/html/html5videotest.html";
	//	mAutoTest = false;
        return openTab(sUrl, false, true, false);
    }

    @Override
    public Tab openIncognitoTab() {
        return openTab(INCOGNITO_URI, false, true, false);
    }
    ...
    private Tab createNewTab(boolean incognito, boolean setActive,
        ...
        if (mTabControl.canCreateNewTab()) {
            ...
            if (useCurrent) {
                tab = mTabControl.getLeastUsedTab(mTabControl.getCurrentTab());
                // Mstar Android Patch Begin
                if (tab == null) {
                    tab = getPrevTab();
                }
...
//          case KeyEvent.KEYCODE_Z:    // unused
    ...
    public boolean dispatchKeyEvent(KeyEvent event) {
        ...
        if (event.getAction() == KeyEvent.ACTION_DOWN &&
            event.getKeyCode() == KeyEvent.KEYCODE_PROG_GREEN &&
            BrowserSettings.getInstance() != null) {

            if (BrowserSettings.getInstance().enableVirtualMouse()) {
                BrowserSettings.getInstance().setVirtualMouseEnabled(false);
            } else {
                BrowserSettings.getInstance().setVirtualMouseEnabled(true);
            }
            mUi.switchToMouseMode(BrowserSettings.getInstance().enableVirtualMouse());
        }
