...
//import android.webkit.WebCusPlayerClient;
...
//import com.android.localmmservice.ILocalMediaService;
//import com.android.localmmservice.ILocalMediaStatusListener;
...
class Tab implements PictureListener {
    ...
    private static final boolean LOGD_ENABLED = com.android.browser.MBrowser.LOGD_ENABLED;
...
/*
// remove dependency on LocalMediaserviice

    private ILocalMediaService mLocalMediaService;
    private ServiceConnection mServiceConnection = new ServiceConnection() {
        public void onServiceConnected(final ComponentName cn, final IBinder service) {
            Log.d(LOGTAG, "LocalMediaService onServiceConnected");
            mLocalMediaService = ILocalMediaService.Stub.asInterface(service);
            try {
                mLocalMediaService.setLocalMediaStatusListener(mLocalMediaStatusListener);
            } catch (RemoteException e) {
                Log.i(LOGTAG, "------- RemoteException ----------");
            } catch (SecurityException e) {
                Log.i(LOGTAG, "------- SecurityException ----------");
            }
        }

        public void onServiceDisconnected(final ComponentName cn) {
            Log.d(LOGTAG, "LocalMediaService onServiceDisconnected");
        }
    };

    private ILocalMediaStatusListener mLocalMediaStatusListener = new ILocalMediaStatusListener.Stub() {
        @Override
        public void mediaSeekCompleted() throws RemoteException {
            Log.d(LOGTAG, "ILocalMediaStatusListener.Stub, mediaSeekCompleted");
        }

        @Override
        public void mediaPrepared() {
            Log.d(LOGTAG, "ILocalMediaStatusListener.Stub, mediaPrepared");
            if (mWebCusPlayerClient != null) {
                mWebCusPlayerClient.handlePrepared();
            }
        }

        @Override
        public void mediaPlayError(int what, int extra)
                throws RemoteException {
            Log.d(LOGTAG, "ILocalMediaStatusListener.Stub, mediaPlayError");
            if (mWebCusPlayerClient != null) {
                mWebCusPlayerClient.handleError(what, extra);
            }
        }

        @Override
        public void mediaPlayInfo(int what, int extra)
                throws RemoteException {
            Log.d(LOGTAG, "ILocalMediaStatusListener.Stub, mediaPlayInfo");
            if (mWebCusPlayerClient != null) {
                mWebCusPlayerClient.handleInfo(what, extra);
            }
        }

        @Override
        public void mediaCompleted() throws RemoteException {
            Log.d(LOGTAG, "ILocalMediaStatusListener.Stub, mediaCompleted");
            if (mWebCusPlayerClient != null) {
                mWebCusPlayerClient.handlePlayCompletion();
            }
        }

        @Override
        public void mediaStop() throws RemoteException {
            Log.d(LOGTAG, "ILocalMediaStatusListener.Stub, mediaStop");
        }

        @Override
        public void videoSizeChanged(int width, int height) throws RemoteException {
            Log.d(LOGTAG, "ILocalMediaStatusListener.Stub, videoSizeChanged");
            if (mWebCusPlayerClient != null) {
                mWebCusPlayerClient.handleVideoSizeChanged(width, height);
            }
        }

        @Override
        public void mediaRelease() throws RemoteException {
            Log.d(LOGTAG, "ILocalMediaStatusListener.Stub, mediaRelease");
        }

        @Override
        public void playExitByUser() throws RemoteException {
            Log.d(LOGTAG, "ILocalMediaStatusListener.Stub, playExitByUser");
            if (mWebCusPlayerClient != null) {
                mWebCusPlayerClient.handlePlayExitByUser();
            }
    ...
    // -------------------------------------------------------------------------
    // WebCusPlayerClient implementation for the main WebView
    // -------------------------------------------------------------------------
    private final WebCusPlayerClient mWebCusPlayerClient = new WebCusPlayerClient() {
         @Override
          public void onSetDataSource(Context context, Uri uri, Map<String, String> headers){
              Log.i(LOGTAG, "WebCusPlayerClient implementation setDataSource uri:"+uri);

             Intent intent = new Intent(MIntent.ACTION_START_MEDIA_BROWSER);
             intent.setData(uri);
             ...
             boolean isBindOk = context.bindService(intent, mServiceConnection, Context.BIND_AUTO_CREATE);
             Log.i(LOGTAG, "WebCusPlayerClient bindService isBindOk:" + isBindOk);
          ...
          @Override
          public void onStart() {
              Log.i(LOGTAG, "WebCusPlayerClient implementation onStart");
          }

          @Override
          public void onStop() {
             Log.i(LOGTAG, "WebCusPlayerClient implementation onStop");
          }

          @Override
          public void onPause() {
              Log.i(LOGTAG, "WebCusPlayerClient implementation onPause");
          }

          @Override
          public  void onSeekTo(int msec) {
              Log.i(LOGTAG, "WebCusPlayerClient implementation onSeek  msec:"+msec);
          }

          @Override
          public void onReset(){
              Log.i(LOGTAG, "WebCusPlayerClient implementation onReset");
          }

          @Override
          public void onRelease(){
              Log.i(LOGTAG, "WebCusPlayerClient implementation onRelease");
          }

          @Override
          public int onGetDuration() {
              Log.i(LOGTAG, "WebCusPlayerClient implementation onGetDuration");
              int getDuration = 0;
              try {
                  if (mLocalMediaService != null) {
                      getDuration = mLocalMediaService.getDuration();
                  }
              } catch(RemoteException e) {
                  Log.i(LOGTAG, "mLocalMediaService.getDuration RemoteException");
              }
              Log.i(LOGTAG, "mLocalMediaService.getDuration:" + getDuration);
              return getDuration;
          }

          @Override
          public int onGetVideoWidth() {
              Log.i(LOGTAG, "WebCusPlayerClient implementation onGetVideoWidth");
              int videoWidth = 0;
              try {
                  if (mLocalMediaService != null) {
                      videoWidth = mLocalMediaService.getWidth();
                  }
              } catch(RemoteException e) {
                  Log.i(LOGTAG, "mLocalMediaService.getVideoWidth RemoteException");
              }
              Log.i(LOGTAG, "WebCusPlayerClient implementation onGetVideoWidth = " + videoWidth);
              return videoWidth;
          }

          @Override
          public int onGetVideoHeight() {
              Log.i(LOGTAG, "WebCusPlayerClient implementation onGetVideoHeight");
              int videoHeight = 0;
              try {
                  if (mLocalMediaService != null) {
                      videoHeight = mLocalMediaService.getHeight();
                  }
              } catch(RemoteException e) {
                  Log.i(LOGTAG, "mLocalMediaService.getVideoHeight RemoteException");
              }
              Log.i(LOGTAG, "mLocalMediaService.getVideoHeight videoHeight = " + videoHeight);
              return videoHeight;
          }

          @Override
          public int onGetCurrentPosition() {
              Log.i(LOGTAG, "WebCusPlayerClient implementation onGetCurrentPosition");
              int currentPosition = 0;
              try {
                  if (mLocalMediaService != null) {
                      currentPosition = mLocalMediaService.getCurrentPosition();
                  }
              } catch(RemoteException e) {
                  Log.i(LOGTAG, "mLocalMediaService.getCurrentPosition RemoteException");
              }
          ...
          public boolean onIsPlaying() {
              Log.i(LOGTAG, "WebCusPlayerClient implementation onIsPlaying");
              boolean isPlaying = false;
              try {
                  if (mLocalMediaService != null) {
                      isPlaying = mLocalMediaService.isPlaying();
                  }
              } catch(RemoteException e) {
                  Log.i(LOGTAG, "mLocalMediaService.isPlaying RemoteException");
              }
              return isPlaying;
          }
    };
*/
    ...
    private final WebChromeClient mWebChromeClient = new WebChromeClient() {
        ...
        public boolean onCreateWindow(WebView view, final boolean dialog,
            ...
            if (dialog && mSubView != null) {
               ...
               */
               ...
               // Do nothing...
               // if can't create a new tab, then find a least used tab to replace of.
            }
        ...
        public void onShowCustomView(View view, int requestedOrientation,
            ...
            if (!mInForeground) mWebViewController.switchToTab(Tab.this);
            if (mInForeground)
    ...
    void setWebView(WebView w, boolean restore) {
        ...
        if (mMainView != null) {
            ...
            /* remove dependency on cusplayerclient
            mMainView.setWebCusPlayerClient(mWebCusPlayerClient);
            if (true == SystemProperties.getBoolean("mstar.browser.cus_player_flow", false)) {
                mMainView.enableCustomerPlayerFlow(true);
            } else {
                mMainView.enableCustomerPlayerFlow(false);
            }
            */
