...
public abstract class BaseUi implements UI ,OnClickListener, OnFocusChangeListener {
    ...
    public BaseUi(Activity browser, UiController controller) {
        ...
        mPrevTabButton = (ImageButton) frameLayout.findViewById(R.id.prevtab_button);
        mLastTabButton = (ImageButton) frameLayout.findViewById(R.id.lasttab_button);
        mLineGuiderButton = (ImageButton) frameLayout.findViewById(R.id.line_guider);
    ...
    public void updateLoadStartTime() {
        ...
        mStartTimeStr = "Start" + getFormatTimeString(mStartTime);
        mTitleBar.mLoadSpeedTextView.setText(mStartTimeStr);
        setLoadSpeedTextViewVisible(true);
    }

    public String getFormatTimeString(long time) {
        Calendar cd = Calendar.getInstance();
        cd.setTimeInMillis(time);
        int hour = cd.get(Calendar.HOUR_OF_DAY);
        int minute = cd.get(Calendar.MINUTE);
        int sec = cd.get(Calendar.SECOND);
        int mis = cd.get(Calendar.MILLISECOND);
        return  String.format(" Time: %s:%s:%s:%s", hour,minute,sec,mis);

    }

    public void updateLoadFinishTime() {
        mFinishTime = System.currentTimeMillis();
        mFinishTimeStr = "Finish" + getFormatTimeString(mFinishTime);
        double timedelta = (double)(mFinishTime - mStartTime)/1000.0;
        DecimalFormat df = new DecimalFormat("##.####");
        String spendTimeMs = df.format(timedelta);

        String str = String.format("%s, %s, Spend Time: %s s", mStartTimeStr,mFinishTimeStr,spendTimeMs);
        mTitleBar.mLoadSpeedTextView.setText(str);
        setLoadSpeedTextViewVisible(true);
    }

    public void setLoadSpeedTextViewVisible(boolean visibled) {
        if (mTitleBar == null) {
            return;
        }
        if(visibled) {
            mTitleBar.mLoadSpeedTextView.setVisibility(View.VISIBLE);
        } else {
            mTitleBar.mLoadSpeedTextView.setVisibility(View.GONE);
        }
    }
