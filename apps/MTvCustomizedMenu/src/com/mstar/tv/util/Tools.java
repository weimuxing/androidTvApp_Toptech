...
public class Tools {
    ...
    public static void releaseTtsEngine() {
        if (tts != null) {
            tts.shutdown();
        }
    }

    public static void setTtsLanguage(Locale locale) {
    ...
    public static void speak(int resId, boolean flushQueue) {
        if (tts != null) {
            if (flushQueue) {
                tts.speak(mContext.getString(resId), TextToSpeech.QUEUE_FLUSH, null);
            }
            else {
                tts.speak(mContext.getString(resId), TextToSpeech.QUEUE_ADD, null);
            }
        }
    }
}
