 ...
 */
public class CloseCaptionBasicSelectionFragment extends SideFragment {
    private final String TAG = "CloseCaptionBasicSelectionFragment";

    private String[] mCCBasicSelectionArray;
    ...
    public class CloseCaptionItem extends RadioButtonItem {
        ...
        public CloseCaptionItem(MainActivity mainActivity, int mode) {
            super(mCCBasicSelectionArray[mode]);
            mMainActivity = mainActivity;
        }

        @Override
        public void onSelected() {
            mSetting.basicMode = (short) getSelectedPosition();
            Log.d(TAG, "Basic selection: " + mSetting.basicMode);
            if (TvCcManager.getInstance() != null) {
                TvCcManager.getInstance().stopCc();
                TvCcManager.getInstance().setCCSetting(mSetting);
                if (TvCcManager.CLOSED_CAPTION_OFF != mSetting.basicMode) {
    ...
    protected String getTitle() {
        return getResources().getString(R.string.str_misc_cc_basic_selection);
    }

    @Override
    protected List<Item> getItemList() {
        List<Item> items = new ArrayList<>();
        mCCBasicSelectionArray = getActivity().getResources().getStringArray(
