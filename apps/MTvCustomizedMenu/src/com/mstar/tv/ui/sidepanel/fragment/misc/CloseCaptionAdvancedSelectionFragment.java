 ...
 */
public class CloseCaptionAdvancedSelectionFragment extends SideFragment {
    private final String TAG = "CloseCaptionAdvancedSelectionFragment";

    private String[] mCCAdvancedSelectionArray;
    ...
    public class CloseCaptionItem extends RadioButtonItem {
        ...
        public CloseCaptionItem(MainActivity mainActivity, int mode) {
            super(mCCAdvancedSelectionArray[mode]);
            mMainActivity = mainActivity;
        }

        @Override
        public void onSelected() {
            mSetting.advancedMode = (short) getSelectedPosition();
            Log.d(TAG, "Advanced selection: " + mSetting.advancedMode);
            if (TvCcManager.getInstance() != null) {
                TvCcManager.getInstance().stopCc();
                TvCcManager.getInstance().setCCSetting(mSetting);
                if (TvCcManager.CLOSED_CAPTION_OFF != mSetting.advancedMode) {
    ...
    protected String getTitle() {
        return getResources().getString(R.string.str_misc_cc_advanced_selection);
    }

    @Override
    protected List<Item> getItemList() {
        List<Item> items = new ArrayList<>();
        mCCAdvancedSelectionArray = getActivity().getResources().getStringArray(
        ...
        Log.d(TAG, "Advanced selection: " + mSetting.advancedMode);
        for (int i = 0; i < mCCAdvancedSelectionArray.length; i++) {
            RadioButtonItem btnItem = new CloseCaptionItem((MainActivity) getActivity(), i);
            if (mSetting.advancedMode == i) {
