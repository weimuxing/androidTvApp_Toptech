...
import com.mstar.tv.ui.sidepanel.SideFragmentManager;
import com.mstar.tv.ui.sidepanel.fragment.AdvancedOptionMenuFragment;
...
/**
 * The main activity for the LiveTV Menu
 */
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mParentalControlSettings = new ParentalControlSettings(this);
        if (getIntent().getAction().equals(Intent.ACTION_MAIN)) {
            mSideFragmentManager.show(new AdvancedOptionMenuFragment());
        } else if (getIntent().getAction().equals("com.mstar.tv.intent.action.DRRT")) {
            mSideFragmentManager.show(new RatingsFragment());
        }
        if (Settings.Secure.getInt(getContentResolver(),
                "speak_in_dtv" , 0) == 1) {
            Log.d(TAG, "Enable TTS Engine!");
            Tools.initTtsEngine(getApplicationContext());
        }
    }
        ...
        if (mKeyQueue.size() == KEYQUEUE_SIZE) {
            ...
            if (keystr.equals(GOODKEYCODES)) {
                if (false == ActivityManager.isUserAMonkey()) {
                    ...
                    List<ResolveInfo> resolveInfo = getPackageManager().queryIntentActivities(intent,
                ...
                if (false == ActivityManager.isUserAMonkey()) {
                    mKeyQueue.clear();
                    Intent intent = new Intent("mstar.tvsetting.factory.intent.action.FactorymenuActivity");
                    List<ResolveInfo> resolveInfo = getPackageManager().queryIntentActivities(intent,
                            PackageManager.MATCH_DEFAULT_ONLY);
                    if (resolveInfo.size() > 0) {
                        startActivity(intent);
